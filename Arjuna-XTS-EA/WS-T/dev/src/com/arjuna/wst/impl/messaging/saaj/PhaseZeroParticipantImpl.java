...
public class PhaseZeroParticipantImpl implements MessageListener, PhaseZeroParticipant, java.io.Serializable
{
    public PhaseZeroParticipantImpl(String serviceURI)
    ...
    public void onMessage(SOAPEnvelope envelope)
    {
        ...
        {
            System.err.println("PhaseZeroParticipantImpl: " + exception);
        }
        catch (Error error)
        {
            System.err.println("PhaseZeroParticipantImpl: " + error);
        }
    }
        ...
        throws MarshalException
    {
        try
        {
            ...
            Name            phaseZeroCompletedName        = envelope.createName(XMLUtil.PHASE_ZERO_COMPLETED_TAG, XMLUtil.WSTX_PREFIX, XMLUtil.WSTX_NAMESPACE_URI);
            SOAPBodyElement phaseZeroCompletedBodyElement = body.addBodyElement(phaseZeroCompletedName);
            SOAPElement     targetProtocolServiceElement  = phaseZeroCompletedBodyElement.addChildElement(XMLUtil.TARGET_PROTOCOL_SERVICE_TAG, XMLUtil.WSTX_PREFIX, XMLUtil.WSTX_NAMESPACE_URI);
            SOAPElement     sourceProtocolServiceElement  = phaseZeroCompletedBodyElement.addChildElement(XMLUtil.SOURCE_PROTOCOL_SERVICE_TAG, XMLUtil.WSTX_PREFIX, XMLUtil.WSTX_NAMESPACE_URI);

            PortReferenceHelper.toXML(envelope, targetProtocolServiceElement, phaseZeroCompletedMessage.targetProtocolService);
            PortReferenceHelper.toXML(envelope, sourceProtocolServiceElement, phaseZeroCompletedMessage.sourceProtocolService);
            if (phaseZeroCompletedMessage.dialogIdentifier != null)
            {
                SOAPElement dialogIdentifierElement = phaseZeroCompletedBodyElement.addChildElement(XMLUtil.DIALOG_IDENTIFIER_TAG, XMLUtil.WSARJTX_PREFIX, XMLUtil.WSARJTX_NAMESPACE_URI);

                dialogIdentifierElement.addTextNode(phaseZeroCompletedMessage.dialogIdentifier);
            }

            SOAPConnection connection = _connectionFactory.createConnection();
            SOAPMessage    responce   = connection.call(message, phaseZeroCompletedMessage.targetProtocolService.getAddress());
        ...
        throws MarshalException
    {
        ...
        try
        {
            ...
            if ((bodyElementName.getLocalName().equals(XMLUtil.PHASE_ZERO_TAG) && bodyElementName.getURI().equals(XMLUtil.WSTX_NAMESPACE_URI)))
            {
                ...
                if (parameters.hasNext())
                {
                    ...
                    if (parameterName.getLocalName().equals(XMLUtil.TARGET_PROTOCOL_SERVICE_TAG) && parameterName.getURI().equals(XMLUtil.WSTX_NAMESPACE_URI))
                        phaseZeroMessage.targetProtocolService = PortReferenceHelper.fromXML(envelope, parameter);
                    else
                ...
                if (parameters.hasNext())
                {
                    ...
                    if (parameterName.getLocalName().equals(XMLUtil.SOURCE_PROTOCOL_SERVICE_TAG) && parameterName.getURI().equals(XMLUtil.WSTX_NAMESPACE_URI))
                        phaseZeroMessage.sourceProtocolService = PortReferenceHelper.fromXML(envelope, parameter);
                    else
                        throw new MarshalException("Unexpected element: " + parameterName.getLocalName() + "; " + parameterName.getURI());
                }
                else
