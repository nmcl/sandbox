 ...
 * UK.
 *
 * $Id: ActivityImple.java,v 1.2 2003/04/04 14:59:51 nmcl Exp $
 */
 ...
 * @version $Id: ActivityImple.java,v 1.2 2003/04/04 14:59:51 nmcl Exp $
 * @since 1.0.
 */
...
public class ActivityImple
{
	...
	synchronized (this)
		...
		throw new WrongStateException("Cannot start activity "+this+" since state is "+_status);
	}
    }
     ...
     *
     * @exception InvalidActivityException Thrown if the current activity is not
     * known about by the activity system.
	 ...
	 * TODO
	 * 
	 * We need an exception that can be thrown to say that the activity
	 * is completing.
	 */
	...
	synchronized (this)
	{
		...
		{
		    /*
		     * Can we do equivalent of rollback on all children
		     * and then rollback this?
		     */

		    throw new InvalidActivityException("Activity "+this+" cannot complete as it has active children.");
		}
		...
		try
		{
			...
			{
			    result = new OutcomeImple(new HLSException(ex), getCompletionStatus());
			}
		...
		{
		    /*
		     * Currently if an exception occurs and we get here,
		     * then we forget all of the other outcomes and
		     * just return the exception. Does this make sense?
		     * How will applications be able to tell which HLSes have
		     * processed the outcome and which have not?
		     */

		    current = new OutcomeImple(new HLSException(ex), Failure.instance());
		}
		...
		else
		{
		    ...
		    throw new WrongStateException("Cannot complete activity in status: "+_status);
		}
	    }
	}
    }
	...
	synchronized (this)
	{
		...
		throw new WrongStateException("Cannot set completion status on "+this+" as status is: "+_status);
	}
    }
    ...
    /**
     * Check whether the specified completion status is compatible with the
     * one currently assigned to the activity.
     *
     * @param CompletionStatus cs The completion status to check.
     *
     * @exception WrongStateException Thrown if the specified status is
     * incompatible with that currently possessed by this activity.
     */
	...
	if (!_completionStatus.equals(cs))
	{
		...
		throw new WrongStateException("Cannot change completion status of "+_completionStatus+" to "+cs+" as incompatible.");
	}
    }
     ...
     *
     * @param ActivityImple child The child activity.
     *
     * @exception WrongStateException Thrown if the parent activity is not
     * in a state that allows children to be added.
     * @exception InvalidActivityException Thrown if the child activity is
     * invalid.
    ...
    {
	if (child == null)
	    throw new InvalidActivityException("Cannot enlist null child!");
	...
	synchronized (this)
	{
		...
		throw new WrongStateException("Cannot enlist child activity with parent as parent's status is: "+_status);
	}
    }
     ...
     *
     * @param ActivityImple child The child activity to remove.
     *
     * @exception WrongStateException Thrown if the parent activity is not
     * in a state that allows children to be removed.
     * @exception InvalidActivityException Thrown if the child activity is
     * invalid.
     * @exception SystemException Thrown if some other error occurs.
     */
    ...
    {
	if (child == null)
	    throw new InvalidActivityException("Cannot remove null child!");
	...
	synchronized (this)
	{
		...
		if (_children.get(child.getGlobalId()) == null)
		    throw new InvalidActivityException("Child activity "+child+" is unknown to parent.");
	    }
	    else
		throw new WrongStateException("Cannot remove child activity from parent as parent's status is: "+_status);		
	}
    }
