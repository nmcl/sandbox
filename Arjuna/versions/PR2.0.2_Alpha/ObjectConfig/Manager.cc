 ...
 * UK.
 *
 * $Id: Manager.cc,v 1.1 1993/08/31 16:47:09 nsmw Exp $
 */
...
LockResult Manager::setlock (Lock *lock, int retry)
{
    LockResult res;
    ...
    if (! multiManager->do_setlock(this, res, lock, retry))
    {
        if (! (configIndex->find(objectUid, multiManager) && multiManager->do_register(this)))
        {
            multiManager = NULL;
...
const Uid& Manager::get_uid () const
{
    return objectUid;
}
