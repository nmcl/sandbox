...
/*
 * $Id: rpcops.cc,v 1.45 1993/10/22 13:23:41 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: rpcops.cc,v 1.45 1993/10/22 13:23:41 nmcl Exp $";
...
int getsn (struct RPC_SN *seq_no)
{
    ...
    seq_no->msec = currentTime; /* time in microseconds */
    return(SUCCEEDED);
}
 ...
 */
  ...
  if ((h_desc = gethostbyaddr((char *) & (host_addr ->log_host),
     ...
     return(NO_HOST_MAP);
  host_addr -> real_host = (char*) h_desc -> h_name;
  return(SUCCEEDED);
...
int clistopen (struct C_LIST_EL *clist)
{
  ...
  if (sm_id >= 0)
  {
      do
      {
	  sem_id = semget(MANSEMKEY, 1, 0600 | IPC_CREAT | IPC_EXCL);
      } while (sem_id < 0);
...
#if defined(hpux) || defined(_AIX)
  sem_ctl = semctl(sem_id, MANSEMKEY, IPC_RMID, 0);
#else
  arg.val = 0;

#ifndef __linux__
  sem_ctl = semctl(sem_id, MANSEMKEY, IPC_RMID, arg);
#else
