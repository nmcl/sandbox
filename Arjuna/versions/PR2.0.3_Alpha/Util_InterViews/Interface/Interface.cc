...
Boolean Interface::Inspect (Buffer& new_state, int &position, int& res)
{
	...
	if ((marker != 0) && (position <= number -1))
	{
	    ...
	    if (B.End() == COMMITTED)
	    {
		new_state.pack(marker->interfaces);
		new_state.pack(marker->hostname);
		new_state.pack(marker->pathname);
		new_state.pack(marker->servers);
...
Boolean Interface::Get (Buffer buff1, Buffer& buff2, int& res)
{
	...
	while (marker)
	{
		...
		if (A.End() == COMMITTED)
		{
		    buff2.pack(marker->hostname);
		    buff2.pack(marker->pathname);
		    return TRUE;
		}
		else
		    return FALSE;
...
Boolean Interface::save_state (ObjectState &os, ObjectType)
{
	...
	while ((x < number) && (ok))
	{
	    ok = os.pack(marker->interfaces) && os.pack(marker->hostname) &&
		os.pack(marker->pathname) && os.pack(marker->servers);
