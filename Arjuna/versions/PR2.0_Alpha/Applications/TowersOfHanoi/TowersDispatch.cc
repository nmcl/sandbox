...
/*
 * $Id: TowersDispatch.cc,v 1.4 1993/03/18 14:08:42 nmcl Exp $
 */
...
TowersDispatch::TowersDispatch()
{
    ...
    RpcControl* RpcConWS = new RpcControl(RpcClientWS);

    if (TowersNew)
	workStack    = new WorkStack(4*(maxDisk + 1), res, RpcConWS);
    else
	workStack    = new WorkStack(res, *WSUid, RpcConWS);
    ...
    RpcControl* RpcConP1 = new RpcControl(RpcClientP1);

    if (TowersNew)
	pinsPersis[0] = new TowerPersis(res, RpcConP1);
    else
	pinsPersis[0] = new TowerPersis(res, *Pin1Uid, RpcConP1);
    ...
    RpcControl* RpcConP2 = new RpcControl(RpcClientP2);

    if (TowersNew)
	pinsPersis[1] = new TowerPersis(res, RpcConP2);
    else
	pinsPersis[1] = new TowerPersis(res, *Pin2Uid, RpcConP2);
    ...
    RpcControl* RpcConP3 = new RpcControl(RpcClientP3);

    if (TowersNew)
	pinsPersis[2] = new TowerPersis(res, RpcConP3);
    else
	pinsPersis[2] = new TowerPersis(res, *Pin3Uid, RpcConP3);
    ...
    RpcControl* RpcConP4 = new RpcControl(RpcClientP4);

    if (TowersNew)
	pinsPersis[3] = new TowerPersis(res, RpcConP4);
    else
	pinsPersis[3] = new TowerPersis(res, *Pin4Uid, RpcConP4);
