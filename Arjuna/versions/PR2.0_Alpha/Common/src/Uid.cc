 ...
 * UK.
 * 
 * $Id: Uid.cc,v 1.20 1993/04/23 14:24:30 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: Uid.cc,v 1.20 1993/04/23 14:24:30 ngdp Exp $";

/*
 * The NIL_UID
 */

const Uid NIL_UID("0.0.0.0:0:0:0");
...
Uid::Uid ( const char *uidString, Boolean errsOk )
{
	...
	/* Pull out inet addr from fist part */
	
	hostAddr = inet_addr(cp);

	/* Extract remaning filds with istream ops */
...
ostream& Uid::print ( ostream& strm ) const
{
    ...
    return strm << inet_ntoa(inet) << ":" << process << ":" << sec << ":" << other;
}
