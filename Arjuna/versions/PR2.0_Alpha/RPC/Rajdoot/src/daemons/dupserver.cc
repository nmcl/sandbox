...
/*
 * $Id: dupserver.cc,v 1.27 1993/05/06 14:08:30 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: dupserver.cc,v 1.27 1993/05/06 14:08:30 nmcl Exp $";
		   ...
		   struct PATHS path[NSTATION], struct MID client_addr)
{
  ...
  if (retry)
  {
    ...
    *sender = (clist -> s_desc[indx]) . sender;
	...
	<< (clist -> s_desc[indx]) . myunicastid . type << ","
	    << (clist -> s_desc[indx]) . myunicastid . hi_part << ","
		<< (clist -> s_desc[indx]) . myunicastid . lo_part << ">\n" << flush;
#endif
    return(ACK_CLIENT);
  }
  else
...
int duplicate_check (int  argc,char **argv)
{
  ...
  case C_LIST_FULL:
    ...
    case PROD_SERVER: 
      ...
      {
	  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	  debug_stream << "dup: sending to sender <" <<  sender.type << ","
		       << sender.hi_part << "," << sender.lo_part << ">\n" << flush;
      }
      else
      {
	  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	  debug_stream << "dup: sending to serverid <" <<  serverid.type << ","
		       << serverid.hi_part << "," << serverid.lo_part << ">\n" << flush;
      }
#endif		      
		      done = FALSE;
                      do
...
int main (int  argc, char **argv)
{
  ...
  myid = getpid();
  sprintf(stdoutfn, "dupserver_out_%ld", myid);
  sprintf(stderrfn, "dupserver_err_%ld", myid);
  ...
  if (strcmp(argv[argc-1], "-d") == 0)
  {
      ...
      if (!target1)
	  printf("duplicateserver: cannot open file dupserver_out\n");
      else
      {	
      ...
      if (!target2)
	  printf("duplicateserver: cannot open file dupserver_err\n");
      else
      {
