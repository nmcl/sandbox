...
/*
 * $Id: clientops.cc,v 1.20 1993/03/18 15:41:25 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: clientops.cc,v 1.20 1993/03/18 15:41:25 nmcl Exp $";
...
{ /* Remove a super group from the list of super groups */
  ...
  debug_stream << "Clientops::REMOVE_SUPER_GROUP(): mygrp <" << mygrp->mymid.type << ","
      << mygrp->mymid.hi_part << "," << mygrp->mymid.lo_part << ">\n" << flush;
#endif
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "<" << current_server->s_id.type << "," << current_server->s_id.hi_part
	  << "," << current_server->s_id.lo_part << ">\n" << flush;
#endif           
    current_server = current_server -> next_serverid;
  }
} /* end traverse_server() */
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "<" << current_group->g_id.type << "," << current_group->g_id.hi_part
	  << "," << current_group->g_id.lo_part << ">\n" << flush;
#endif           
    current_group = current_group -> next_groupid;
  }
} /* end traverse_group() */
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "SUPERGROUP: <" << mygrp->mymid.type << "," << mygrp->mymid.hi_part
	  << "," << mygrp->mymid.lo_part << "> contains " << mygrp->grps.no_grps << " groups, "
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "\nSUPERGROUP:: <" << current_supergrp->mymid.type << ","
	  << current_supergrp->mymid.hi_part << "," << current_supergrp->mymid.lo_part << "> contains"
...
void set_grp_info (struct GROUPS *group, struct MID groupid)
{
  ...
  group->grp_srvr.srvr.sr_id.type = 0;
  group->grp_srvr.srvr.sr_id.hi_part = 0;
  group->grp_srvr.srvr.sr_id.lo_part = 0;
