 ...
 * UK.
 *
 * $Id: AtomicA.cc,v 1.28 1993/04/27 14:44:42 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: AtomicA.cc,v 1.28 1993/04/27 14:44:42 ngdp Exp $";
...
#ifdef HAVE_INTSIG
int critAAsignalHandler ( int DOTDOTDOT )
#else
void critAAsignalHandler ( int DOTDOTDOT  )
#endif
{
#ifdef DEBUG
    debug_stream << TRIVIAL_FUNCS << FAC_ATOMIC_ACTION << VIS_PRIVATE;
    debug_stream << "critAAsignalHandler\n" << flush;
#endif
	...
	currAct->setHandler(critAAsignalHandler);
    }
}
...
#ifdef HAVE_INTSIG
int nonCriAAsignalHandler ( int DOTDOTDOT )
#else
void nonCritAAsignalHandler ( int DOTDOTDOT )
#endif
{
#ifdef DEBUG
    debug_stream << TRIVIAL_FUNCS << FAC_ATOMIC_ACTION << VIS_PRIVATE;
    debug_stream << "nonCritAAsignalHandler\n" << flush;
#endif
    ...
    currAct->setHandler(nonCritAAsignalHandler);
}
...
/*
 * Refined version of the restore_state operation that would typically
 * be invoked during crash recovery. THIS CODE IS CURRENTLY BROKEN
 */

Boolean AtomicAction::restore_state ( ObjectState&, ObjectType )
{
    /* 
     * Ought to do some processing of the intention_list to recreate
     * the various records, although this would be better left
     * to another program....
     */

    error_stream << WARNING << " Invocation of Atomic_Action::restore_state"
	         << " for action " << get_uid() << " ignored\n";
    return (TRUE);
}

/*
 * Refined version of save_state inherited from StateManager which
 * saves the intention list as the state of the object.
 * This operation will be invoked to ensure the intention list
 * is uneffected by a node crash.
 */
...
Boolean AtomicAction::save_state ( ObjectState& os, ObjectType ot )
{
	...
	if ((temp->type_is() == PERSISTENCE) || (temp->type_is() == RPCCALL))    // Silly But True.
...
void AtomicAction::criticalEnd ()
{
    ...
    /* 
     * change the current signal handler for SIGINT to the
     * non_critical_signal_handler
     */
 ...
 * by setting a new signal handler. If this new signal handler is
...
void AtomicAction::criticalStart ()
{
    ...
    setHandler(critAAsignalHandler);
}
...
void AtomicAction::actionInitialise ()
{
	...
	setHandler(nonCritAAsignalHandler);
    }
