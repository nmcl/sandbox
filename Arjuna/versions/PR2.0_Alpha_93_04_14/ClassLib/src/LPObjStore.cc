 ...
 * was made with the SHADOW FileType argument.
 */
					  ...
					  const int lMode )
{
	...
	if ((cachePtr != 0) && (cachePtr->cachedDictPtr != 0))
	{
		...
		if (::fstat(fd, &statarg) == 0)
		{
		    ...
		    {
			cachePtr->defaultBlockSize = statarg.st_blksize;
			cachePtr->maxCachedImageSize = statarg.st_blksize - sizeof(struct StateDictionary);
		    }
...
void LocalPersistentObjectStore::storeLocation ( const char* )
{
}
