 ...
 * UK.
 *
 * $Id: LockMan.cc,v 1.32 1993/03/22 12:15:48 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: LockMan.cc,v 1.32 1993/03/22 12:15:48 ngdp Exp $";
...
void LocalLockManager::propagate ( const Uid& actionUid )
{
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_CONCURRENCY_CONTROL << VIS_PUBLIC;
    debug_stream << "LocalLockManager::propagate(" << actionUid << ")\n" << flush;
#endif
    ...
    if (loadState())
    {
	/* should only be callable in action environment - if not just return */
    
	if (currAct != 0)
	{
	    AtomicAction *myparent = currAct->parent();
	    ...
	    {
		if (current->getOwner() == actionUid)
		{
		    current->setOwner(myparent->get_uid(), current->isAction());
