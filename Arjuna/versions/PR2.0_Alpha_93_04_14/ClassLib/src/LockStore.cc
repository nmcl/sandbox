 ...
 * UK.
 *
 * $Id: LockStore.cc,v 1.19 1993/04/30 12:53:37 nsmw Exp $
 */
...
static const char RCSid[] = "$Id: LockStore.cc,v 1.19 1993/04/30 12:53:37 nsmw Exp $";
...
/*
 * Create the lock store. This creates or initialises the control segment but the actual
		       ...
		       mapTablePtr(0)
{
    ...
    debug_stream << "LockStore::LockStore(" << key << ")\n" << flush;
#endif
	...
	error_stream << WARNING << "Failed to allocate shared memory for"
	             << " lock storage - (increase system limits?)\n";
}
...
LockStore::~LockStore ()
{
    ...
    debug_stream << "LockStore::~LockStore()\n" << flush;
#endif
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_OBJECT_STORE << VIS_PUBLIC;
    debug_stream <<"LockStore::remove(" << u << ", " << tname << ")\n" << flush;
#endif
	...
	if (mapIndex >= 0)
	{
		...
		if (useSeg != 0)
		    ...
		    error_stream << WARNING
				 ...
				 << segmentIndex << " failed" << endl << flush;
	    }
	    else
		return TRUE;
	}
	...
	if (mapIndex >= 0)
	{
		...
		if (useSeg != 0)
		{
			...
			if (!(useSeg->putState(slotIndex, state)))
			{
			    error_stream << WARNING 
					 << "LockStore::write_state() : "
					 << "failed to write into lock store\n" << flush;
			    return FALSE;
			}
	    ...
	    error_stream << WARNING 
			 ...
			 << u << " of type " << tName << endl << flush;
    }
    else
	error_stream << WARNING 
		     << "LocalStore::write_state() segment with id " 
			 << shmid() << " is marked invalid\n" << flush;
...
LockSegment *LockStore::attachSegment ( long segIndex )
{
    ...
    debug_stream << "LockStore::attachSegment(" << segIndex << ")\n" << flush;
#endif
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_OBJECT_STORE << VIS_PRIVATE;
    debug_stream << "LockStore::allocateSegment(" << slotSize << ")\n" << flush;
#endif
