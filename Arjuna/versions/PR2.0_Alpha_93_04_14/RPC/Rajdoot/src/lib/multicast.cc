...
/*
 * $Id: multicast.cc,v 1.28 1993/05/05 13:41:12 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: multicast.cc,v 1.28 1993/05/05 13:41:12 ngdp Exp $";
...
int initialise_client_ipc ()
{
	...
	mc_hdr.source.lo_part = (unsigned long) uni_name.sin_port;

#ifdef DEBUG
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "Socket " << client_ipc << " port number " << uni_name.sin_port << "\n" << flush;
#endif
    }
...
int multicast_send (struct MID *midptr, struct mc_msghdr *msg)
{
	...
	case UID_type:
	    ...
	    hostport.sin_port = (unsigned short) midptr->lo_part;
...
int multicast_receive (struct MID *midptr, struct mc_sg_filter *msg_ptr, int (*filter) (struct mc_sg_filter *, Boolean), long toval)
{
	...
	if (FD_ISSET(client_ipc, &watch))  /* Standard socket receive */
	{
	    ...
	    msg_ptr->source.type = (unsigned long) htonl(UID_type);
	    msg_ptr->source.hi_part = (unsigned long) from.sin_addr.s_addr;
	    msg_ptr->source.lo_part = (unsigned long) from.sin_port;
	}
	...
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << ", port " << from.sin_port << "\n" << flush;
#endif
