...
/*
 * $Id: rpc.cc,v 1.56 1993/05/07 11:23:54 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: rpc.cc,v 1.56 1993/05/07 11:23:54 nmcl Exp $";
...
int default_filter (struct mc_sg_filter *new_msg, Boolean timeout)
{
  ...
  debug_stream << "default filter: and source address :" << flush;
...
int initiate (struct OBJECT_DESC *objects, struct GROUPS *newgroup, int *successes)
{
  ...
  long   pathlength,pid, serverport, packed_size;
  int    i, group_number, opn_code = NOARG;
  int    indx, rcode, xrstat, xretry, rretry; 
  int    no_failures, max_failures;
...
#ifdef DEBUG
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << "rpc: initiate multiaddr: " << flush;
  printgid(multithem);
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << " and actid " << flush;
  printgid(actid);
#endif
  ...
  unithem . type = htonl((long)UID_type);
  unithem . lo_part = htons((unsigned short) manager_port);
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "rpc: trying to send to manager at " << flush;
      printgid(unithem);
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "action is " << flush;
      printgid(actid);
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
...
#ifdef DEBUG
	 debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	 debug_stream << "rpc: must UNICAST - send ok waiting to receive on " << flush;
	 printgid(client);
#endif
	 do
	 {
	     xrstat = multicast_long_receive(&client, & ack, 0, GWTIMEOUT); 
	 } while (xrstat == ENOMESSAGE);
      }
#ifdef DEBUG
      else
      {
...
#ifdef DEBUG
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "rpc: received from server: " << flush;
	printgid(sender);
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "rpc: server port set to " << srvr_reply.lo_part << "\n" << flush;
#endif            
    }       
    else
    {  
	       ...
	       long opcode, struct OBJECT_DESC *objects)
{
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "group_opn: sending opcode " << rq_code << "\n" << flush;
    debug_stream << "group_opn: no of acks expected from servers: " << acks_expected << "\n" << flush;
    debug_stream << "rpc: group_opn: trying to send to server <" << transmit_group.type
	<< "," << transmit_group.hi_part << "," << transmit_group.lo_part << ">\n" << flush;
#endif
  }
  else  /* for 1 server */
    acks_expected = 1;
  ...
  if (xstat == SUCCEEDED)
  {
	...
	debug_stream << "rpc: opcode was " << rs_code << " TERM_OK(" << TERM_OK << ")" << flush;
      ...
      ((isserver) && (acks_expected <= 0)))
  {
      if (termcall.mc_msg->mc_msg[0].iov_base)
      {
	  free(termcall.mc_msg->mc_msg[0].iov_base);
	  termcall.mc_msg->mc_msg[0].iov_base = 0;
      }
      if (termcall.mc_msg->mc_msg[1].iov_base)
      {
	  free(termcall.mc_msg->mc_msg[1].iov_base);
	  termcall.mc_msg->mc_msg[1].iov_base = 0;
      }
      return(SUCCEEDED);
  }
		...
		long retry, struct FILTER_INFO *filter, struct OBJECT_DESC *objects)
{
  ...
  req_buf . mc_msg[4] . iov_len  = (int) packed_size;             // ** wrong size !! **
      ...
      ((destn->grp_srvr.group).head_gd->sids.no_srs == 1)))
  {
    ...
    /* timeout for each call  - converted from centisecs to millisecs */
      /* Don't need to do this now as long_receive converts time */
/*    t_out = ((timeout * 10) / (retry + 1) + ROUNDTRIP_TIME);*/
	...
	else
	{
	  ...
	  if (multicast_long_receive(&client, &ack, 0, t_out) > 1)
	  {
	    ...
	    debug_stream << "multi_call: seqnos are: <expected> <returned> <" << ntohl(call_seqno.sec)
		<< "," << ntohl(call_seqno.msec) << "> <" << ntohl(result_seqno.sec) << ","
		    << ntohl(result_seqno.msec) << ">\n" << flush;
#endif
	    ...
	    { /* Unicasting, only 1 seqno to check, can be done outside filter */
	      ...
	      result->rpc_buff1 = result_buf.mc_msg[2].iov_base;
	      result->rpc_length1 = result_buf.mc_msg[2].iov_len;
	      result->rpc_buff2 = result_buf.mc_msg[3].iov_base;
		...
		struct MID group, struct OBJECT_DESC *objects)
{
  ...
  else
  {
    ...
    unithem . type = htonl((long)UID_type);
    unithem . lo_part = htons((unsigned short) MANAGER_ADDR);
 ...
 */
  ...
  if (myhost == 0)
  {
    if (gethostname(r_host, sizeof(r_host)-1) == 0)
    {
