...
/*
 * $Id: rpcops.cc,v 1.39 1993/04/29 11:56:48 nmcl Exp $
 */
...
extern unsigned long get_machine_id();
...
void insert_rpc_nextlist (struct HEAD_RPCLIST *, int *, struct RPC_LIST *,
			 struct RPC_LIST *);
void insert_rpcinfo (struct HEAD_RPCLIST *, long, long, char *, long, char *);
...
static const char RCSid[] = "$Id: rpcops.cc,v 1.39 1993/04/29 11:56:48 nmcl Exp $";
...
char *pack_group (struct GROUPS *group, int *packedsize)
{
  ...
  case UNSET_type:
    ...
    *packedsize = sizeof(group->groups_type);
    ...
    buff = start_buff;
    pack_long(group->groups_type, buff);
    break;
  ...
  case SRVR_type:
    ...
    *packedsize = sizeof(group->groups_type) + sizeof(struct MID);
    start_buff = (char*) malloc(*packedsize);
    buff = start_buff;
    buff = pack_long(group->groups_type, buff);
  ...
  case GRP_type:
    ...
    *packedsize = sizeof(group->groups_type) * 2;
    ...
    buff = pack_long(group->groups_type, buff);
    buff = pack_long(no_supergrps, buff);
    ...
    {
      *packedsize += (sizeof(struct MID) + sizeof(long));
	...
	current_g = current_g->next_groupid;
      ...
      *packedsize += (sizeof(long) + (sizeof(struct MID) * 2 * no_servers));

      if ((start_buff = (char*) realloc((malloc_t) start_buff, *packedsize)) == NULL) 
	return(NULL);
      buff = start_buff + offset;

      buff = pack_long(no_servers, buff);
...
struct GROUPS *unpack_group (char *group, int  packedsize)
{
  ...
  long no_supergrps, no_groups, no_servers;
  int sgindx, gindx, sindx;
  char * buff;

  if (packedsize < sizeof(long))
    return(NULL);

  buff = group; /* Initialise temporary buffer */
  unpackedgroup = (struct GROUPS *) malloc(sizeof(struct GROUPS));
  
  buff = unpack_long(&(unpackedgroup->groups_type), buff);
  ...
  case GRP_type:
    ...
    buff =  unpack_long(&no_supergrps, buff);
    ...
    do
    {
      ...
      buff = unpack_long(&no_groups, buff);
	...
	insert_groups(&(unpackedgroup->grp_srvr.group),
      ...
      buff = unpack_long(&no_servers, buff);
...
Boolean identical_sn (struct RPC_SN *sn1, struct RPC_SN *sn2)
{
    ...
    debug_stream << "with <" << sn2->sec << ", " << sn2->msec << ">\n" << flush;
#endif
...
int getsn (struct RPC_SN *seq_no)
{
    ...
    seq_no->sec = htonl((get_machine_id() << 16) | (getpid() & 0xffff));
    seq_no->msec = htonl(currentTime); /* time in microseconds */
    return(SUCCEEDED);
}
 ...
 */
  ...
  if ((h_desc = gethostbyaddr((char *) & (host_addr ->log_host),
     ...
     return(NO_HOST_MAP);
  host_addr -> real_host = h_desc -> h_name;
  return(SUCCEEDED);
  
} /* end getrealhost() */    

void setalarm (unsigned long limit)
{
...
char *pack_long (long  _val, char *_buff)
{
	*(long *) _buff = _val;
...
char *unpack_long (long *_val, char *_buff)
{
	*_val = *(long *) _buff;
...
char *pack_MID (struct MID _val, char *_buff)
{
	...
	_buf = _buff;
	_buf = pack_long(_val.type, _buf); 
	_buf = pack_long(_val.hi_part, _buf);
	_buf = pack_long(_val.lo_part, _buf);
...
char *unpack_MID (struct MID *_val, char *_buff)
{
	...
	_buf = _buff;
	_buf = unpack_long(&_val->type, _buf); 
	_buf = unpack_long(&_val->hi_part, _buf);
	_buf = unpack_long(&_val->lo_part, _buf);
...
char *pack_PATHS (struct PATHS _val, char *_buff)
{
  ...
  _buf = _buff;
  _buf = pack_long(_val.node, _buf);
  _buf = pack_long(_val.c_count, _buf);
...
char *unpack_PATHS (struct PATHS *_val, char *_buff)
{
  ...
  _buf = _buff;
  _buf = unpack_long(&_val->node, _buf);
  _buf = unpack_long(&_val->c_count, _buf);
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "<" << gid.type << "," << gid.hi_part << "," << gid.lo_part << ">\n" << flush;
#endif
}

void set_rpcinfo (struct RPC *new_rpc, long opcode, long blength1, char *buffer1, long blength2, char *buffer2)
{
...
int f_clistopen (int *f_clistfd, struct C_LIST_EL *clist)
{
    struct flock arg;     
    ...
    if ((*f_clistfd = open(C_LIST_Q, O_RDWR, 0666)) < 0)
    {
	_rpcerr = OPENFAILED;
	return(FAILED);
    }

    if (fcntl(*f_clistfd, F_SETLKW, (int)&arg) < 0)
    {
	perror("clist Error");
...
int clistopen (struct C_LIST_EL *clist)
{
  _rpcerr = 0;
  errno = 0;
  ...
  if (sm_id >= 0)
  {
      do
      {
	  sem_id = semget(2000, 1, 0600 | IPC_CREAT | IPC_EXCL);
      } while (sem_id < 0);
...
int f_clistclose (int *f_clistfd, struct C_LIST_EL *clist)
{
    struct flock arg;
    ...
    if ((fcntl(*f_clistfd, F_SETLKW, (int)&arg) < 0) || (close(*f_clistfd) < 0))
    {
	_rpcerr = CLOSEFAILED;
	return(FAILED);
    }
...
int clistclose (struct C_LIST_EL *clist)
{
#ifndef hpux
  union semun arg;
#endif
...
#ifndef hpux
  arg.val = 0;
  sem_ctl = semctl(sem_id, 2000, IPC_RMID, arg);
#endif
#ifdef hpux
  sem_ctl = semctl(sem_id, 2000, IPC_RMID, 0);
#endif
  if (sem_ctl == -1)
  {
      cerr << "Failed to remove lock. Errno " << errno << "\n" << flush;
      _rpcerr = CLOSEFAILED;
      return(FAILED);
  }
...
int get_clientindx (struct C_LIST_EL *clist, long client_node)
{
...
void RmClientFromPath (struct C_LIST_EL *clist, int client_indx, long client_node)
{
...
#ifdef DEBUG
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << "\ndec_filtgrp: received from server: " << flush;
  printgid(server);
#endif
...
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "\ndec_filtgrp: servers in list: " << flush;
...
#ifdef DEBUG
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << "after creating rpclist: currentlist " << flush;
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "\nafter constructing alt lists: currentlist: " << flush;
    traverse_rpclist(currentlist);
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "next list " << flush;
...
void insert_newmsg (struct mc_sg_filter *new_msg, struct HEAD_RPCLIST *new_rpc)
{
  ...
  insert_rpcinfo(new_rpc,
		 * (long *) new_msg->mc_msg->mc_msg[1].iov_base,
		 ...
		 new_msg->mc_msg->mc_msg[2].iov_base, 
...
void insert_rpcinfo (struct HEAD_RPCLIST *new_rpc, long opcode, long blength1, char *buffer1, long blength2, char *buffer2)
{
