...
/*
 * $Id: rpcserver.cc,v 1.40 1993/03/18 15:41:53 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: rpcserver.cc,v 1.40 1993/03/18 15:41:53 nmcl Exp $";
...
static SERVERSWORK  latest_work;
static int  latest_indx;
...
int default_server_filter (struct mc_sg_filter *new_msg, Boolean timeout)
{
  ...
  debug_stream << "default filter: received source address: " << flush;
 ...
 */
	...
	if (latest_work[indx].result.rpc_buff2)
      ...
      if (multicast_long_send(&dest_group, &res_buf) != MC_OK)
      {
...
int log_list (struct C_LIST_EL *pclist, int nodindx, long client_node, struct MID mymid, struct MID myadd, long myprocessid, struct MID actid, struct UNIQUEID uid, struct RPC_SN clock)
{
  ...
  debug_stream << "rpcs; logged in at indx, " << logindx << " serverid: <"
      << pclist -> s_desc[logindx] . myunicastid . type << ","
	  << pclist -> s_desc[logindx] . myunicastid . hi_part << ","
	      << pclist -> s_desc[logindx] . myunicastid . lo_part << ">\n" << flush;
#endif
...
int check_mid (struct MID givenmid)
{
  ...
  while ((i <= latest_group) && (!found))
  {
#ifdef DEBUG
      debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
      debug_stream << "rpcs: mygroups[" << i << "]: <" << mygroups[i].type << ","
	  << mygroups[i].hi_part << "," << mygroups[i].lo_part << ">" << flush;
      debug_stream << " and givenmid: <" << givenmid.type << "," << givenmid.hi_part << ","
	  << givenmid.lo_part << "\n" << flush;
#endif
    ...
    {
      found = TRUE;
      mygroups[i].hi_part = mygroups[i].lo_part = 0;
    }
    i++;
  }
...
int init_server (int argc, char **argv)
{
  ...
  sm_id = shmget(1000, sizeof(struct C_LIST_EL), 0666);
...
int check_client (struct RPC_SN *seqno, struct MID *source_address)
{
  ...
  { /* client not found */
    ...
    do
    {
      if (latest_work[indx].client_address.lo_part == (short) UNSET_type)
        found = TRUE;
      else
	indx++;
    } while ((indx <= latest_indx) && (!found));
...
int getwork (struct GROUPS *client_address, struct RPC *srvr_work, struct DEADLINE *deadlines, struct FILTER_INFO *filter)
{
  ...
  for(;;)
  {
...
#ifdef DEBUG
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "receiving on <" << myunicastid.type << "," << myunicastid.hi_part << ","
	    << myunicastid.lo_part << ">\n" << flush;
#endif
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "rpcserver: received opcode: " << htonl(opn_code) << "\n" << flush;
#endif
	...
	debug_stream << "rpcs: about to join group <" << givenmid.type << "," << givenmid.hi_part
	    << "," << givenmid.lo_part << ">\n" << flush;
...
#ifdef DEBUG
	  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	  debug_stream << "rpcserver: about to terminate server\n" << flush;
#endif
...
#ifdef DEBUG
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "rpcs: received terminate all message\n" << flush;
#endif
