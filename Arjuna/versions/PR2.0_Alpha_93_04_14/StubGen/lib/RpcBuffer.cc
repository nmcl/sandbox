 ...
 * UK.
 *
 * $Id: RpcBuffer.cc,v 1.12 1993/05/06 14:49:43 nsmw Exp $
 */

static const char RCSid[] = "$Id: RpcBuffer.cc,v 1.12 1993/05/06 14:49:43 nsmw Exp $";
...
/* Externally callable operators for some Common classes */

RpcBuffer& operator<< ( RpcBuffer& rpcbuff, const Buffer *buff )
{
    if (rpcbuff.packPtr(buff))
	buff->packInto(rpcbuff);
...
RpcBuffer& operator>>  ( RpcBuffer& rpcbuff, Uid *& tounpack )
{
    ...
    {
	if (tounpack == 0)
	    tounpack = new Uid(NIL_UID);
	tounpack->unpack(rpcbuff);
    }
    else
	tounpack = (Uid *)rpcbuff.pointerAt(index);
    return rpcbuff;
}

RpcBuffer& operator>> ( RpcBuffer& rpcbuff, Buffer *& buff )
{
    void *p;
    long index;
	...
	buff->unpackFrom(rpcbuff);
        ...
        buff = (Buffer *)rpcbuff.pointerAt(index);
