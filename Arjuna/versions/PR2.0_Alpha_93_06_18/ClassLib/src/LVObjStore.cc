						...
						FileType ft)
{
    ...
    {
        error_stream << WARNING << "Could not find state to remove" << endl;
					      ...
					      off_t& off_set)
{
    ...
    {
        error_stream << WARNING << "Virtual object store full." << endl;
	(void)unlockObjectStore();
	return FALSE;
    }
...
Boolean LocalVolatileObjectStore::pack (Buffer& buff)
{
    ...
    if (!openAndLock())
    {
        error_stream << WARNING << "openAndLock failed" << endl;
	return FALSE;
    }
