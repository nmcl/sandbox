 ...
 * UK.
 *
 * $Id: LockMan.cc,v 1.6 1994/03/08 09:19:32 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: LockMan.cc,v 1.6 1994/03/08 09:19:32 ngdp Exp $";
...
Boolean LocalLockManager::propagate ( const Uid& from, const Uid& to )
{
	...
	error_stream << WARNING
		     << "LockManager::propagate() lock propagation failed (state load failed)" << endl;
	freeState();
				       ...
				       unsigned int sleepTime )
{
	...
	{
	    error_stream << WARNING
		<< "LockManager::setlock() cannot load existing lock states" << endl;
	    break;
	}
...
{ 
    ...
    debug_stream << "LocalLockManager::doRelease(" << u << ", " << oneOrAll << ")" << endl;
#endif
...
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_CONCURRENCY_CONTROL << VIS_PRIVATE;
    debug_stream << "LocalLockManager::isAncestorOf(" << heldLock.get_uid() << ")" << endl;
#endif
