 ...
 * UK.
 *
 * $Id: SharedSeg.cc,v 1.3 1993/11/16 09:55:47 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: SharedSeg.cc,v 1.3 1993/11/16 09:55:47 ngdp Exp $";
...
char *SharedSegment::smInit ( const key_t smKey, size_t smSize )
{
	    ...
	    ((segId = ::shmget(smKey, (int) size, 0666)) == -1))
	{
            error_stream << WARNING 
			 << "SharedSegment::SharedSegment() failed: ";
            if (savedErrno != EEXIST)
		error_stream << " Could not create segment ";
	    else
		error_stream << " Could not attach segment";
    ...
    {
        error_stream << WARNING 
		     << "SharedSegment::attach() failed" << endl;
	...
	{
            error_stream << WARNING 
			 << "SharedSegment::SharedSegment() : Segment size mismatch" << endl;
...
void SharedSegment::detach ( int shmId )
{
	...
	if (p->usage == 0)
	{
	    /* Usage now zero check if should remove from system */
