		       ...
		       mapTablePtr(0)
{
	...
	if (mapTablePtr->segmentInitialised)
	{
	    ...
	    {
		if (mapTablePtr->segmentTable[i].smId != NULL_SEGMENT_NO)
...
LockSegment *LockStore::attachSegment ( long segIndex )
{
    ...
    if ((lockSegs[segIndex] == 0) &&
	(mapTablePtr->segmentTable[segIndex].smId != NULL_SHMID))
    {
	LockSegment *t = new LockSegment(mapTablePtr->segmentTable[segIndex].smId);
