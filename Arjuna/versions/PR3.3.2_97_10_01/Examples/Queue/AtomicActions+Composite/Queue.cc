 ...
 * UK.
 *
 * $Id: Queue.cc,v 1.3 1994/09/02 11:32:18 ngdp Exp $
 */
...
QueueStatus Queue::enqueue (int v)
{
	...
	if (numberOfElements <= QUEUE_SIZE)
	{
	    ...
	    tmp = new QueueLinks;
	    tmp->data = new QueueElement(v, result);
...
Boolean Queue::restore_state (ObjectState& os, ObjectType)
{
	...
	if (res)
	{
	    res = FALSE;
	    tmp = new QueueLinks;
	    tmp->data = new QueueElement(u, res);
