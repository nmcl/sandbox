...
/*
 * $Id: longmltcst.cc,v 1.15.4.1.2.1 1996/10/10 12:26:18 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: longmltcst.cc,v 1.15.4.1.2.1 1996/10/10 12:26:18 ngdp Exp $";


#ifdef SYSV
extern double clockTick;
#endif
...
void setBurstSize (Uint32 toSet)
{
    burstSize = toSet;
}

void getBurstSize (Uint32& toSet)
{
    toSet = burstSize;
}

void setBurstTime (Uint32 toSet)
{
    burstTime = toSet;
}

void getBurstTime (Uint32& toSet)
{
    toSet = burstTime;
}
...
Uint32 numb_packets (struct mc_msghdr *message, Uint32 *msgsize)
{
    Uint32 packet_size;
    Int32 packets;
    Uint32 msize = 0;

    packet_size = MAX_SIZE - (sizeof(Int32) * message->msg_len) - sizeof(struct info);
...
int multicast_long_send (struct MID *midptr, struct mc_msghdr *message)
{
    ...
    if ((!midptr) || (ntohl(midptr->lo_part) < 0) || (ntohl(midptr->type) == UNSET_type))
    ...
    debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
    debug_stream << "multicast_long_send: sending " << ntohl(header.size) << " bytes" << endl;
    debug_stream.unlock();
...
/* which specify timeout period, given in centi-seconds.                     */
/*****************************************************************************/

int multicast_long_receive (struct MID *midptr, struct mc_sg_filter *msg_ptr,
			    int (*filter) (struct mc_sg_filter *, Boolean), Int32 time_out)
{
    ...
    start_ms = (Uint32) stime.tv_sec*1000000 + stime.tv_usec;
#endif
	...
	do
	{
		...
		{
		    length = ntohl(header.total) * MAX_SIZE;

		    /* Has to be first packet. */

		    if (ntohl(header.total) > 1)
			buffer = (caddr_t) realloc((malloc_t) buffer, (unsigned int) length);
		...
		interval = (Int32)((time_out * 10000) - convertTime);
#else		
		gettimeofday(&ftime, 0);
		finish_ms = (Uint32) ftime.tv_sec * 1000000 + ftime.tv_usec;
		interval = (time_out * 10000) - (finish_ms - start_ms);
#endif
	    }

	    // do not indicate a timeout if we got a message at the last moment.
	    
	    if ((rstat < 0) && (interval < 0) && (time_out != -1))
		rstat = ETIMEOUT;
	...
	if (rstat > 0)
	{
	    Int32 sizeToUse = ntohl(header.unknown.field1) + ntohl(header.unknown.field2);

	    if (ntohl(header.unknown.field3) > 0)
		msg_ptr->mc_msg->mc_msg[msg_ptr->mc_msg->msg_len -3].iov_len = (int) ntohl(header.unknown.field3);
	    else
		if (msg_ptr->mc_msg->msg_len > 2)
		    msg_ptr->mc_msg->mc_msg[msg_ptr->mc_msg->msg_len -3].iov_len = 0;
	    ...
	    if ((ntohl(header.unknown.field2) > 0) || (ntohl(header.unknown.field1) > 0))
	    {
...
int multicast_send_message (struct OBJECT_DESC *objects, struct mc_msghdr *message)
{
    ...
    while (objects[i].destn_host != 0)        /* Use multiple unicast */
    {
	  ...
	  do
	  {
	      t_status = multicast_long_send(&objects[i].grps_id.grp_srvr.srvr.sr_id, message);
