...
/*
 * $Id: multicast.cc,v 1.12.4.1.2.1 1996/10/10 12:26:20 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: multicast.cc,v 1.12.4.1.2.1 1996/10/10 12:26:20 ngdp Exp $";
...
int initialise_client_ipc ()
{
	...
	mc_hdr.source.hi_part = (Uint32) uni_name.sin_addr.s_addr;
	mc_hdr.source.lo_part = htonl((Int32) ntohs(uni_name.sin_port));
...
void multicast_terminal_closedown (char *logfile)
{
    ...
    for (long ii = sysconf(_SC_OPEN_MAX) - 1; ii >= 0; --ii)
	close((int) ii);
...
int multicast_join (struct MID *midptr)
{
	...
	case NOT_JOINED:
	    ...
	    if (recvfrom(mcrdaemon_ipc, &ack, sizeof(ack), 0, (struct sockaddr *)&mcrdaemon_addr, &fromlen) > 0)
	    {
...
int multicast_send (struct MID *midptr, struct mc_msghdr *msg)
{
	...
	case UID_type:
	    ...
	    hostport.sin_port = htons((short) ntohl(midptr->lo_part));
	    hostport.sin_family = AF_INET;

	    buffer.msg_name = (caddr_t) &hostport;
	    buffer.msg_iov = (struct iovec *) &msg->mc_msg[0];
	    break;
	default:		/* Cannot send to anything other than UID_type, JOINED or NOT_JOINED */
	    return ECANTSEND;
    }
...
int multicast_receive (struct MID *midptr, struct mc_sg_filter *msg_ptr, int (*filter) (struct mc_sg_filter *, Boolean), Int32 toval)
{
	...
	if (FD_ISSET(client_ipc, &watch))  /* Standard socket receive */
	{
	    ...
	    msg_ptr->source.hi_part = (Uint32) from.sin_addr.s_addr;
...
void multicast_new_mid (struct MID *newmidptr)
{
    ...
    newmidptr->lo_part = (Int32) htonl(currentTime); /* time in microseconds */
}
