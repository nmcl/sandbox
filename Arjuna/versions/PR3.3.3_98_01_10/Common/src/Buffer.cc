 ...
 * UK.
 *
 * $Id: Buffer.cc,v 1.21.6.1 1996/10/10 12:25:36 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: Buffer.cc,v 1.21.6.1 1996/10/10 12:25:36 ngdp Exp $";
...
Boolean Buffer::pack ( unsigned long long ll )
{
    ...
    {
	/* BEWARE writeHeader WILL CHANGE packPoint arbitrarily */
	
	Uint64 u64 = ll;
	char *oldPackPoint = packPoint;	/* remember */
	
	longlongSize = sizeof(unsigned long long);
	
	if (writeHeader())
	{
	    packPoint = oldPackPoint;
...
void Buffer::marshall ( RpcBuffer& buff ) const
{
	...
	if (len != 0)
	{
	    buff.packMappedBlock(bufferStart, len);
	}
}
			    ...
			    Boolean shouldDel )
{
	...
	if ((::memcmp(bufferStart, VERSION, sizeof(VERSION)) == 0))
	{
	    ...
	    if ((shortSize != sizeof(unsigned short)) ||
		 ...
		 (longSize != sizeof(unsigned long)))
		{
		    /* Basic sizes ok watch out for long long */

		    if (bufflonglongSize != 0)
		    {
			/* buffer contains long longs - make sure I can read them */

			longlongSize = bufflonglongSize;
#ifdef HAVE_LONG_LONG
			if (bufflonglongSize != sizeof(unsigned long long))
#endif
			    readOnly = TRUE;
		    }
		}
...
Boolean Buffer::writeHeader ()
{
    if (bufferStart != 0)
    {
	size_t needed = sizeof(VERSION) + sizeof(Uint32);

	if ((length() >= needed) || sizeup(needed))
	{
	    Uint32 ui = shortSize;
