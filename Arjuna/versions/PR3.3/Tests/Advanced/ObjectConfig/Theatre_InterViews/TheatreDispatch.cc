...
/*
 * $Id: TheatreDispatch.cc,v 1.1 1993/11/11 15:18:59 nmcl Exp $
 */
...
TheatreDispatch::TheatreDispatch() : TheatreInter(num_seats, this)
{
    ...
    if (TheatreNew)
	ThPersis = new TheatrePersis(num_seats, num_seats, res);
    else
	ThPersis = new TheatrePersis(res, *TheatreUid);
...
void TheatreDispatch::MenuDo(enum DispatchOper oper_num)
{
    ...
    case clusterasseats_oper:
    {
        int res = 0;

        SetOperation("Cluster As Seats");
	SetOperResult("");

        ThPersis->ClusterAsSeats(res);
        if (res == 0)
             SetOperResult("Not Done");
        else
             SetOperResult("Done");
	...
	break;
    case clusterasrows_oper:
    {
        int res = 0;

        SetOperation("Cluster As Rows");
	SetOperResult("");

        ThPersis->ClusterAsRows(res);
        if (res == 0)
             SetOperResult("Not Done");
        else
             SetOperResult("Done");
	...
	break;
    case clusterastheatre_oper:
    {
        int res = 0;

        SetOperation("Cluster As Theatre");
	SetOperResult("");

        ThPersis->ClusterAsTheatre(res);
        if (res == 0)
             SetOperResult("Not Done");
        else
             SetOperResult("Done");
