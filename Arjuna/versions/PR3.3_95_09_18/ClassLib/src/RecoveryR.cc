 ...
 * UK.
 *
 * $Id: RecoveryR.cc,v 1.11 1995/06/26 13:14:16 ngdp Exp $
 */
...
static const char RCSid[] = "$Id: RecoveryR.cc,v 1.11 1995/06/26 13:14:16 ngdp Exp $";

Boolean RecoverySetUp::setUp = FALSE;
...
RecoverySetUp::RecoverySetUp ()
{
    ...
    debug_stream << "RecoverySetUp::RecoverySetUp ()" << endl;
    debug_stream.unlock();
    ...
    {
        RecoveryRecord tr;
	AbstractRecord::addToList(RecoverySetUp::create, RecoverySetUp::remove, tr.type());
	setUp = TRUE;
    }
}
...
AbstractRecord* RecoverySetUp::create ()
{
    return new RecoveryRecord;
}

void RecoverySetUp::remove (AbstractRecord*& toDelete)
{
    delete toDelete;
    toDelete = 0;
}
...
Boolean RecoveryRecord::save_state ( ObjectState& os, ObjectType ot)
{
    ...
    return ( (order().pack(os)
	      && os.packNonMappedString(getTypeOfObject())) ? TRUE : FALSE );
#else
