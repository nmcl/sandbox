...
/*
 * $Id: dupserver.cc,v 1.24 1996/10/09 13:10:42 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: dupserver.cc,v 1.24 1996/10/09 13:10:42 nmcl Exp $";
...
{
  int client_indx, freeslot, j, k, ptindx = 0;
  Boolean found = FALSE;
  ...
  while((ptindx < NSTATION) && (path[ptindx].node != 0))
  {
	...
	while ((j < NSTATION) && (freeslot < 0))
	{
      ...
      if (freeslot < 0)
      {
	  error_stream << WARNING << "dupserver - could not find free slot in clist." << endl;	  
	  return(C_LIST_FULL);
      }
      else
      {
	...
	/* Reject initiate request since it is from a previous incarnation of client */
...
void log_path (struct C_LIST_EL *clist, struct PATHS path[NSTATION], int server_indx, int link_indx,
	       struct RPC_SN timestamp, struct MID client_addr)
{
  int pt_indx = 0;

  while (path[pt_indx].node != 0)
  {
    clist->s_desc[server_indx].linkspath[link_indx].rpcpath[pt_indx] = path[pt_indx];
    pt_indx++;
  }
...
int inspect_clist (struct C_LIST_EL *clist, struct RPC_SN timestamp, struct MID actid, struct UNIQUEID uid,
		   Int32 clientnode, struct MID *serverid, struct MID *sender, Int32 *serverpid,
		   struct PATHS path[NSTATION], struct MID client_addr)
{
  ...
  else
  {
	...
	m++;
      ...
      if (!inserted)
      {
	  error_stream << WARNING << "dupserver - could not insert client in c_list: no room." << endl;
	  return(C_LIST_FULL);
      }
      else
	return(PROD_SERVER);
...
int modify_clist ()
{
    int result = FAILED, retry = FILESRETRY, rstat;
...
int duplicate_check (int argc, char **argv)
{
  ...
  Boolean             done;
  Int32                client_port, pathlength;
  Int32                sropcode, rs_code, error, serverpid, junk;
  ...
  debug_stream.lock();
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << "dupserver: exec args are :";
  for (i = 0; argv[i] != 0; i++)
      debug_stream << argv[i] << " ";
  debug_stream << endl; 	
  debug_stream.unlock();
  ...
  sscanf(argv[10], "%d", &actid . type );
  sscanf(argv[11], "%d", &actid . hi_part);
  sscanf(argv[12], "%d", &actid . lo_part);
  sscanf(argv[13], "%d", &uid . host);
  sscanf(argv[14], "%d", &uid . sec);
  sscanf(argv[15], "%d", &uid . msec);

  /* Buffers 16 and 17 reserved for group man opn_code and deaf server */
  sscanf(argv[16], "%d", & junk);
  sscanf(argv[17], "%d", & junk);
  
  /* Get rpc-path length */
  sscanf(argv[18], "%d", & pathlength);
  pathlength = ntohl(pathlength);
  i = 19;
  ptr = path;
  while(i < argc -2)
  {   /* Get rpc-path */
    sscanf(argv[i++], "%d", & ptr->node);
    sscanf(argv[i++], "%d", & ptr->c_count);
  ...
  printgid(actid);
  debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
  debug_stream << "dup: uid <" << uid.host << "," << uid.sec << "," << uid.msec << ">" << endl;
  debug_stream << "dup: pathlength " << pathlength << " about to log path " << endl;
    for (int j = 0; j < pathlength; j++)
    {
	debug_stream << FUNCTIONS << FAC_BASIC_RPC << VIS_PUBLIC;
	debug_stream << "<" << path[j].node << "," << path[j].c_count << ">" << endl;
    }
  debug_stream.unlock();
#endif
  ...
  case C_LIST_FULL:
	...
	debug_stream.unlock();
                          ...
                          if (xrstat > 0)
			  {
			    ...
			    cerr << "duplicate_check: send failed" << endl;
#endif
...
//                      kill ((int) serverpid, INTERRUPT_SERVER);
	...
	debug_stream.unlock();
		      ...
		      debug_stream.unlock();
                      ...
                      argp[0] = server_name;
                      sprintf(argp[1], "%d", n_client_addr . type);
                      sprintf(argp[2], "%d", n_client_addr . hi_part);
                      sprintf(argp[3], "%d", client_port);
                      sprintf(argp[4], "%d", n_lastrpcsn . sec);
                      sprintf(argp[5], "%d", n_lastrpcsn . msec);
                      sprintf(argp[6], "%d", multi_addr . type);
                      sprintf(argp[7], "%d", multi_addr . hi_part);
                      sprintf(argp[8], "%d", multi_addr . lo_part);
                      sprintf(argp[9], "%d", f_clistfd);
                      if (debug_mode)
			  argp[10] = "-d";
                      else
			  argp[10] = "+d";
			  ...
			  if (errno == ENOENT)
			  {
			      error_stream << WARNING << "dupserver execv error - no server "
					   << full_name << endl;
			      senderror(&n_client_addr, NOSERVER);
			  }
			  else
			  {
			      if (errno == EACCES)
				  ...
				  error_stream << WARNING
					       << "dupserver execv error - server access permission problem for "
					       << full_name << endl;
				  senderror(&n_client_addr, SERVERACCESS);
			      }
			      else
			      {
				  senderror(&n_client_addr, FAILED);
				  error_stream << WARNING
					       << "duplicateserver: failed to exec server " << full_name
					       << " - errno " << errno << endl;
			  ...
			  modify_clist();  // only close now, since we modify in place.
		      }
...
int main (int  argc, char **argv)
{
  ...
  myid = getpid();
  sprintf(tempOfn, "_out_%d", myid);
  sprintf(tempEfn, "_err_%d", myid);
