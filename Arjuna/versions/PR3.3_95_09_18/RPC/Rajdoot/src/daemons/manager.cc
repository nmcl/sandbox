...
/*
 * $Id: manager.cc,v 1.41 1996/12/17 18:03:09 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: manager.cc,v 1.41 1996/12/17 18:03:09 nmcl Exp $";
		   ...
		   struct UNIQUEID *uid, Int32 opn_code, Int32 df_serverport)
{
  ...
  if (!(child_id = fork()) )
  {
	...
	pathlength++;
      ...
      if (forceTermination)
	  argp[i+1] = "-t"; // force termination is idle, i.e., no orphan detection
      else
	  argp[i+1] = "+t"; // use orphan detection to terminate
      argp[i+2] = (char *) NULL;
...
int main (int argc, char **argv)
{
	...
	while (j < argc)
	{
		...
		case 't':
		    ...
		    break;
		case 'h':
		    printf("Usage: manager [-h] [-v] [-s] [-p] [-d] [-t]\n");
		    return 0;
		    break;
		default:
