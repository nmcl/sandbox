...
/*
 * $Id: rpcserver.cc,v 1.35 1996/10/09 13:12:22 nmcl Exp $
 */
...
static Boolean p_orphan = FALSE;   /* optimization on looking up clist to 
				    ...
				    */
static Boolean forceTermination = FALSE;  // auto terminate if idle, i.e., no orphan detection?
static void (*Dump)() = 0;
  ...
  for(;;)
  {
	...
	debug_stream << "receiving on <" << ntohl(myunicastid.type) << "," << ntohl(myunicastid.hi_part) << ","
	  ...
	  if (n_received == ETIMEOUT) // only do this if a timeout has occurred with no message received.
	  {
	      ...
	      {
		  error_stream << WARNING << "Server < " << serverName << ", " << getpid()
			       << " > auto-terminating." << endl;
		  
		  LeaveAllGroups();
		  cleanupclist();
		  tidyUp();

		  kill(getpid(), ORPHAN_SIGNAL);
	      }
	  }
	  else
	  {
