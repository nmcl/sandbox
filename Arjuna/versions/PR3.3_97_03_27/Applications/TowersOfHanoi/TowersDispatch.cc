...
/*
 * $Id: TowersDispatch.cc,v 1.9 1996/11/07 17:41:56 nsmw Exp $
 */
...
TowersDispatch::TowersDispatch(int isReplicated)
{
    ...
    RpcConWS = new ClientRpcManager(RpcClientWS);

    if (TowersNew)
    {
	workStack    = new WorkStack(4*(maxDisk + 1), res, RpcConWS);
        cout << "WorkStack (-uidws) Object: " << workStack->get_uid() << endl;
    }
    else
    ...
    RpcConP1 = new ClientRpcManager(RpcClientP1);

    if (TowersNew)
    {
	pinsPersis[0] = new TowerPersis(res, RpcConP1);
        cout << "Pin1 (-uid1) Object      : " << pinsPersis[0]->get_uid() << endl;
    }
    else
    ...
    RpcConP2 = new ClientRpcManager(RpcClientP2);

    if (TowersNew)
    {
	pinsPersis[1] = new TowerPersis(res, RpcConP2);
        cout << "Pin2 (-uid2) Object      : " << pinsPersis[1]->get_uid() << endl;
    }
    else
    ...
    RpcConP3 = new ClientRpcManager(RpcClientP3);

    if (TowersNew)
    {
	pinsPersis[2] = new TowerPersis(res, RpcConP3);
        cout << "Pin3 (-uid3) Object      : " << pinsPersis[2]->get_uid() << endl;
    }
    else
    ...
    RpcConP4 = new ClientRpcManager(RpcClientP4);

    if (TowersNew)
    {
	pinsPersis[3] = new TowerPersis(res, RpcConP4);
        cout << "Pin4 (-uid4) Object      : " << pinsPersis[3]->get_uid() << endl;
    }
    else
