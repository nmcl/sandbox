 ...
 * UK.
 *
 * $Id: ActionH.cc,v 1.15 1997/03/27 10:24:55 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: ActionH.cc,v 1.15 1997/03/27 10:24:55 nmcl Exp $";
				   ...
				   currentDepth(copy.currentDepth)
{
	...
	for (unsigned int i = 0; i < maxHierarchyDepth; i++)
	    hierarchy[i] = 0;	
    }
    
    for (unsigned int i = 0; i < currentDepth; i++)
	hierarchy[i] = new ActionInfo(*copy.hierarchy[i]);
}
...
ActionHierarchy& ActionHierarchy::operator= ( const ActionHierarchy& copy )
{
    ...
    for (unsigned int j = 0; j < currentDepth; j++)
	hierarchy[j] = new ActionInfo(*copy.hierarchy[j]);

    for (unsigned int k = currentDepth; k < maxHierarchyDepth; k++)
	hierarchy[k] = 0;
...
Boolean ActionHierarchy::pack ( Buffer& state ) const
{
	...
	for (unsigned int i = 0; (i < currentDepth) && result; i++)
	{
	    ...
	    if (result)
		result = state.pack(hierarchy[i]->actionType);
	}
    }
