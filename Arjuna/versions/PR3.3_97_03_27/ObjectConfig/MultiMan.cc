 ...
 * UK.
 *
 * $Id: MultiMan.cc,v 1.3 1994/10/21 12:28:20 nsmw Exp $
 */
...
Boolean MultiManager::do_setlock (Manager* manager, LockResult& lockRes, Lock *lock, int retry, unsigned int sleepTime)
{
#ifdef DEBUG
    debug_stream << FUNCTIONS << FAC_OBJECT_CLUSTERING << VIS_PUBLIC;
    ...
    {
        lockRes = LockManager::setlock(lock, retry,  sleepTime);
        res = TRUE;
    }
...
{
#ifdef DEBUG
    debug_stream << CONSTRUCTORS << FAC_OBJECT_CLUSTERING << VIS_PUBLIC;
...
{
#ifdef DEBUG
    debug_stream << DESTRUCTORS << FAC_OBJECT_CLUSTERING << VIS_PUBLIC;
    debug_stream << "MultiManager::~MultiManager ()\n" << flush;
#endif
