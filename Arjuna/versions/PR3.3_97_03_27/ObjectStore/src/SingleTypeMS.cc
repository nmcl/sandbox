...
SingleTypeMappedStore::~SingleTypeMappedStore ()
{
	...
	{
	    error_stream << WARNING << "SingleTypedMappedStore::~SingleTypedMappedStore - mutex wait failed." << endl;
	}
    }
...
Boolean SingleTypeMappedStore::writeStates ()
{
	...
	error_stream << WARNING
		     << "SingleTypeMappedStore::writeStates - write failed"
		     << endl;
    }
    else
    {
...
Boolean SingleTypeMappedStore::readStates ()
{
    ...
    {
	error_stream << WARNING
		     << "SingleTypeMappedStore::readStates - read failed"
		     << endl;
    }
					    ...
					    off_t& off_set )
{
	...
	for (int i = 0; (!found) && (marker != 0); i++)
	{
        ...
        error_stream << WARNING
		     << "SingleTypeMappedStore - store full." << endl;
						...
						ObjectStore::StateType ft)
{
	...
	if (tn)	
	{
	    ...
	    {
		error_stream << WARNING
			     << "SingleTypeMappedStore::read_state - Attempt to read shadow from non-shadowed store."
			     << endl;
	    }
	}
	else
	{
	    error_stream << WARNING
			 << "SingleTypeMappedStore::read_state - Object with uid "
					      ...
					      const TypeName tn )
{
	...
	{
	    error_stream << WARNING
			 << "SingleTypeMappedStore::commit_state - object with uid "
			 << objUid << " has no TypeName\n";
	}
    }
					      ...
					      ObjectStore::StateType ft )
{
	...
	{
	    error_stream << WARNING << "SingleTypeMappedStore::remove_state - type() operation of object with uid "
			 << objUid << " returns NULL";
	}
    }
