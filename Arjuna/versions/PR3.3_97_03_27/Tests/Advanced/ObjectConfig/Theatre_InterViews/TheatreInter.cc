...
/*
 * $Id: TheatreInter.cc,v 1.1 1993/11/11 15:19:03 nmcl Exp $
 */
...
TheatreInter::TheatreInter(int& numSeats, TheatreDispatch* ThDisp)
{
    ...
    PulldownMenu* configmenu = new PulldownMenu("Configuration ");
    configmenu->Include(new MenuItemDispatch("Cluster As Seats",   TheatreDisp, clusterasseats_oper));
    configmenu->Include(new MenuItemDispatch("Cluster As Rows",    TheatreDisp, clusterasrows_oper));
    configmenu->Include(new MenuItemDispatch("Cluster As Theatre", TheatreDisp, clusterastheatre_oper));
    menubar->Include(configmenu);
