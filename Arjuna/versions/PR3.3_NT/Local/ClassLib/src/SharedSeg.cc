...
SharedSegment::SharedSegment ( int smId )
                             : segId(smId),
#else
SharedSegment::SharedSegment ( const key_t smId)
							 : segId(smId),
							   segName(0),
							   fileHandle(0),
							   segHandle(0),
...
key_t SharedSegment::shmid () const
{
	return segId;
}
...
HANDLE SharedSegment::openExisting (key_t smKey)
{
	Uid u;
	...
	segName = ::new char[MAX_PATH];
	::memset(segName, '\0', MAX_PATH);
	...
	if (smKey == -1)
	{
		...
		sprintf(segName, "c:/temp/SegmentFile_%s", uidBuff);
	}
	else	
		sprintf(segName, "c:/temp/SegmentFile_%d", smKey);

	/*
	 * File is opened and/or created.
	 */

	segHandle = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, segName);
...
char *SharedSegment::smInit ( const key_t smKey, size_t smSize )
{
	...
	if ((fileHandle = CreateFile(segName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
								0, OPEN_ALWAYS, 0, 0)) == INVALID_HANDLE_VALUE)
	{
		error_stream << WARNING << "SharedSegment CreateFile " << segName << " failed." << endl;
		return 0;
	}
	 ...
	 * existing one. If we have just created the file then CreateFileMapping will grow
	 * it to the size we really need, i.e., we don't need to explicitly do a write
	 * to the file to initialise it.
	 */

	segHandle = CreateFileMapping(fileHandle, NULL, PAGE_READWRITE, 0, size, segName);
 ...
 * and if it drops to zero check the usage count on the segment
 * itself. If that is zero actually remove the segment from the system.
 * Since we should have acquired a mutex at a higher level, we can assume
 * that it is safe to remove the file.
 */
