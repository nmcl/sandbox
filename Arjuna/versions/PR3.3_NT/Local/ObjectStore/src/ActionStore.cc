...
SetUpInfo* ActionStoreSetUp::setup = 0;
				   ...
				   : ShadowingStoreSetUp(NULL)
{
    ...
    if (!ActionStoreSetUp::setup)
    {
		ActionStoreSetUp::setup = new SetUpInfo();

		ActionStoreSetUp::setup->_new = &createASInstance;
		ActionStoreSetUp::setup->_tn = ActionStoreType;
		ActionStoreSetUp::setup->_setUp = TRUE;
		ObjectStore::addToList(ActionStoreSetUp::setup);
    }
#endif
}
