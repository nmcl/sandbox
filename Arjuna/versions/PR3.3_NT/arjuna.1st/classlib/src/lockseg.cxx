...
Lock_Segment::Lock_Segment ( key_t k, size_t slotSize )
                         : BaseSegment()
{
#ifdef DEBUG
    debug_stream.lock();
    ...
    debug_stream << "Lock_Segment::Lock_Segment("
	         << slotSize << ")" << endl;
    debug_stream.unlock();
...
#else
	if (smInit(-1, (unsigned int)(allocSize*slotCount)) != 0)
#endif
    {
...
/*
 * Create new Lock_Segment manager for existing segment
 */

#ifndef WIN32
Lock_Segment::Lock_Segment ( int lockSegId )
                         : BaseSegment(lockSegId)
{
    ...
    debug_stream << CONSTRUCTORS << FAC_SHMEM << VIS_PUBLIC;
    debug_stream << "Lock_Segment::Lock_Segment(" << lockSegId
	         << ")" << endl;
    debug_stream.unlock();
...
Lock_Segment::Lock_Segment ( const char* lockSegId )
                         : BaseSegment(lockSegId)
{
#ifdef DEBUG
    debug_stream.lock();
...
Lock_Segment::~Lock_Segment ()
{
    ...
    debug_stream << "Lock_Segment::~Lock_Segment()" << endl;
    debug_stream.unlock();
...
Boolean Lock_Segment::getState ( const Uid& u,
								...
								long slotNo ) 
{
    ...
    debug_stream << "Lock_Segment::getState(" << u << ", " 
	         << tName << ")" << endl;
    debug_stream.unlock();
	...
	if (slotNo > 0)
	{
		...
		temp << "Lock_Segment::getState() : corrupt state header in slot: " << slotNo
		     << " for segment: " << shmid()
	...
	{
	    temp << "Lock_Segment::getState() : invalid slot number " << slotNo
		 << ends;
...
/*
 * remove for the Lock_Segment class simply clears the slot
...
Boolean Lock_Segment::freeSlot ( long slotNo )
{
#ifdef DEBUG
    debug_stream.lock();
    
    debug_stream << FUNCTIONS << FAC_SHMEM << VIS_PUBLIC;
    debug_stream <<"Lock_Segment::freeSlot(" << slotNo << ")" << endl;
    debug_stream.unlock();
	...
	if (slotNo > 0)
	{
	    ...
	    {
		temp << "Lock_Segment::freeSlot() : corrupt state in slot: " << slotNo
	...
	{
	    temp << "Lock_Segment::freeSlot() : invalid slot number " << slotNo
		 << ends;
	    addError(new Error(FAC_OBJECT_STORE, LS_BAD_SLOT,buff));
	}
    }
...
Boolean Lock_Segment::putState ( long slotNo,
			        const ObjectState& state )
{
    ...
    debug_stream << FUNCTIONS << FAC_SHMEM << VIS_PUBLIC;
    debug_stream <<"Lock_Segment::putState("  << slotNo << ", "
	         << (void *)&state << ")" << endl;
    debug_stream.unlock();
	...
	if ((stateSize + sizeof(SlotHeader)) <= segZero.slotSize)
	{
		    ...
		    (b.guard == redZone.guard))
		{
		    ...
		    {
				if (put(offset + sizeof(SlotHeader), b.stateSize, state.buffer()) != (long) b.stateSize)
				{
		...
		{
		    /* State header is corrupt */
		    temp << "Lock_Segment::putState() : corrupt state header in slot: " << slotNo
			 << " for segment: " << shmid()
			 << hex << " guard: " << b.guard << dec << " size: " << b.stateSize << ends;
	    ...
	    addError(new Error(FAC_OBJECT_STORE, OS_TOO_BIG,
			       "Lock_Segment::putState() : state to large for slot\n"));
    }
...
long Lock_Segment::allocateSlot ( )
{  
    if (isValid())
    {
...
Boolean Lock_Segment::hasSpareSlots ()
{
    if (getSegVaddr() != 0)
    {
	/* read control info */
...
Boolean Lock_Segment::checkIfConsistent ()
{
    Boolean result = TRUE;
    int inUse = 0;
