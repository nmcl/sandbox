...
SetUpInfo* ShadowingStoreSetUp::setup = 0;
...
#ifdef RUNTIME_STATIC_INIT
    if (!ShadowingStoreSetUp::setup)
    {
		ShadowingStoreSetUp::setup = new SetUpInfo();
		ShadowingStoreSetUp::setup->_new = &createSSInstance;
		ShadowingStoreSetUp::setup->_tn = ShadowStoreType;
		ShadowingStoreSetUp::setup->_setUp = TRUE;
		ObjectStore::addToList(ShadowingStoreSetUp::setup);
    }
#endif
}
...
#else
			if ((fd = openAndLock(fname, O_RDONLY | SYNC | O_BINARY, GENERIC_READ)) != -1)
#endif
			{
    ...
    if (name)
    {
		...
		if ((state == OS_COMMITTED) || (state == OS_UNCOMMITTED))
		{
			...
			if (::access(fname, 2) == -1)
			{
				...
				if (ft == ObjectStore::OS_ORIGINAL)
					error_stream << WARNING
						 << "ShadowingStore::remove_state() - access problems on "
						 << objUid << "\n";
#endif
			}
			else
				if (::unlink(fname) == -1)
				{
					...
					if (ft == OS_ORIGINAL)
						error_stream << WARNING 
								 << "ShadowingStore::remove_state() - unlink failed on "
								 << fname << "\n";
				}
		...
		else
		{
			...
			if (state == OS_UNKNOWN)
				...
				error_stream << "hidden ";
			error_stream << "state for object with uid " << objUid << endl;
		}
    }
    else
    {
										...
										ObjectStore::StateType ft ) 
{
    ...
    if (tName)
    {
		char *fname = genPathName(objUid, tName, ft);
 		int imageSize = state.size();

#ifndef WIN32
		int fd = openAndLock(fname, O_CREAT|O_WRONLY|SYNC, F_WRLCK);
#else
		int fd = openAndLock(fname, O_CREAT|O_WRONLY|SYNC|O_BINARY, GENERIC_WRITE);
#endif
		
		if (fd == -1)
		{
			error_stream << WARNING 
						 << "ShadowingStore::write_state() - openAndLock "
					 << "failed for " << fname << "\n";
			delete [] fname;
			return(FALSE);
		}
		...
		if (imageSize > 0)
		{
			...
			{
				error_stream << WARNING 
						 << "ShadowingStore::write_state() - write failed "
						 << "for " << fname << "\n";
				delete [] fname;
				return(FALSE);
			}
				...
				{
					error_stream << WARNING
						 << "ShadowingStore::write_state() - fsync failed "
						 << "for " << fname << "\n";
					delete [] fname;
					return(FALSE);
				}
...
#ifdef WIN32
			/*
			 * We need to do this because rename will not overwrite an existing file.
			 */

			if (ft == ObjectStore::OS_SHADOW)
				remove_state(objUid, tName, ObjectStore::OS_ORIGINAL);
#endif
		}
		...
		error_stream << WARNING << "Object with uid " << objUid << " has no TypeName\n";
