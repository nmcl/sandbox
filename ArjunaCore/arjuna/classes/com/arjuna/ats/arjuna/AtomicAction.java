 ...
 * UK.  
 *
 * $Id: AtomicAction.java,v 1.2 2003/07/03 14:09:37 nmcl Exp $
 */
 ...
 *
 * An instance of this class is a transaction that can be started
 * and terminated (either committed or rolled back). There are also
 * methods to allow participants (AbstractRecords) to be enlisted with
 * the transaction and to associate/disassociate threads with the
 * transaction.
 *
 * @author Mark Little (mark@arjuna.com)
 * @version $Id: AtomicAction.java,v 1.2 2003/07/03 14:09:37 nmcl Exp $
 * @since JTS 1.0.
 */
...
public class AtomicAction extends TwoPhaseCoordinator
{
     ...
     * at this point.
     *
     * No timeout is associated with this transaction, i.e., it will not
     * be automatically rolled back by the system.
     */
     ...
     * an error code will be returned. No timeout is associated with the
     * transaction.
     *
     ...
     *
     * @param int timeout the timeout associated with the transaction. If
     * the transaction is still active when this timeout elapses, the
     * system will automatically roll it back.
     *
     * @return <code>ActionStatus</code> indicating outcome.
     */

    public synchronized int begin (int timeout)
    {
     ...
     * are reported.
     *
     * If the transaction has already terminated, or has not
    ...
    /**
     * The type of the class is used to locate the state of the transaction
     * log in the object store.
     *
     * Overloads BasicAction.type()
     *
     * @return a string representation of the hierarchy of the class for
     * storing logs in the transaction object store.
     */
    ...
    /**
     * Register the current thread with the transaction. This operation is
    ...
    /**
     * Register the specified thread with the transaction. This operation is
     * not affected by the state of the transaction.
     *
     * @return <code>true</code> if successful, <code>false</code> otherwise.
     */
    ...
    /**
     * Unregister the current thread from the transaction. This operation is
     * not affected by the state of the transaction.
     *
     * @return <code>true</code> if successful, <code>false</code> otherwise.
     */
     ...
     *
     * If the current transaction is not an AtomicAction then this method
     * will not suspend.
     *
     * @return a handle on the current AtomicAction (if any) so that the
     * thread can later resume association if required.
     *
     * message com.arjuna.ats.atomicaction_1 Attempt to suspend a non-AtomicAction transaction. Type is {0}
     */
	...
	if (curr != null)
	{
		...
		{
		    tsLogger.arjLoggerI18N.warn("com.arjuna.ats.arjuna.atomicaction_1",
						new Object[]{curr.toString()});
		}
     ...
     * will be lost.
     *
     * @param AtomicAction act the transaction to associate. If this is a
     * nested transaction, then the thread will be associated with all of the
