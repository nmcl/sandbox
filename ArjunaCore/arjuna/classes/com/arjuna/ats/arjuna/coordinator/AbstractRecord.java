 ...
 * UK.  
 *
 * $Id: AbstractRecord.java,v 1.3 2003/08/11 15:04:13 nmcl Exp $
 */
 ...
 * @version $Id: AbstractRecord.java,v 1.3 2003/08/11 15:04:13 nmcl Exp $
 * @since 1.0.
 */
...
public final boolean equals (AbstractRecord ar)
    {
	return ((com.arjuna.ats.arjuna.common.Configuration.useAlternativeOrdering()) ? typeEquals(ar) : orderEquals(ar));
    }
...
public final boolean lessThan (AbstractRecord ar)
    {
	return ((com.arjuna.ats.arjuna.common.Configuration.useAlternativeOrdering()) ? typeLessThan(ar) : orderLessThan(ar));
    }
...
public final boolean greaterThan (AbstractRecord ar)
    {
	return ((com.arjuna.ats.arjuna.common.Configuration.useAlternativeOrdering()) ? typeGreaterThan(ar) : orderGreaterThan(ar));
    }
...
public int nestedCleanup ()
    ...
    /**
     * Should this record be saved in the intentions list? If the record
     * is saved, then it may be recovered later in the event of a failure.
     * Note, however, that the size of the intentions list on disk is
     * critical to the performance of the system (disk I/O is a bottleneck).
     *
...
public String type ()
     ...
     *
     * @param PrintWriter strm the stream on which to output.
     */
...
public void print (PrintWriter strm)
    ...
    /**
     * When the transaction is required to make the intentions list
     * persistent, it scans the list and asks each record whether or not
     * it requires state to be saved (by calling doSave). If the answer is
     * yes, then save_state is called and the record instance must save
     * enough information to enable it to be restored from that state
     * later. The basic AbstractRecord save_state will save common
     * data that is required by the base class during recovery.
     *
...
public boolean save_state (OutputObjectState os, int i)
    ...
    /**
     * During recovery, the transaction log is given to the recovery
     * system and it will recreate a transaction instance to perform
     * necessary recovery actions. This transaction will recreate the
     * intentions list and give each recreated AbstractRecord the state
     * that that was saved during transaction persistence. The base class
     * will restore information that it needs from the log.
     *
     * @return <code>true</code> if successful, <code>false</code>
     * otherwise.
     */
...
public boolean restore_state (InputObjectState os, int i)
     ...
     * then the instance must be remembered by the transaction (in the log)
     * in order for recovery to retry later or for a system administrator
     * to be able to determine which resources have not been successfully
     * completed.
     */
...
public int topLevelOnePhaseCommit ()
    ...
    /**
     * @return a newly created instance of the derived class specified by
     * the ClassName, or null if there is nothing defined in the inventory.
...
public static AbstractRecord create (ClassName cName)
     ...
     *
     * @param AbstractRecord a the record with which to merge.
     */
...
public abstract void merge (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a the record with which to alter.
     */
...
public abstract void alter (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a The record to try to add.
...
public abstract boolean shouldAdd (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a The record to try to alter.
...
public abstract boolean shouldAlter (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a The record to try to merge.
...
public abstract boolean shouldMerge (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a The record to try to replace.
...
public abstract boolean shouldReplace (AbstractRecord a);
     ...
     *
     * @param AbstractRecord a the record that will replace this instance.
     */
...
protected final AbstractRecord getNext ()
     ...
     *
     * @param AbstractRecord ar the instance to become previous.
     */
...
protected final void setPrevious (AbstractRecord ar)
     ...
     *
     * @param AbstractRecord ar the instance to become next.
     */
...
protected final void setNext (AbstractRecord ar)
     ...
     *
     * @param Uid storeUid the unique id for this instance.
     * @param String objType the type of the instance.
     * @param int otype the ObjectType of the object.
...
protected AbstractRecord (Uid storeUid, String objType, int otype)
	...
	if (tsLogger.arjLogger.debugAllowed())
	{
	    tsLogger.arjLogger.debug(DebugLevel.CONSTRUCTORS, VisibilityLevel.VIS_PROTECTED,
				     ...
				     "AbstractRecord::AbstractRecord ("+storeUid+", "+otype+")");
	}
    }
...
protected AbstractRecord (Uid storeUid)
     ...
     *
     * @message com.arjuna.ats.arjuna.coordinator.AbstractRecord_1 [com.arjuna.ats.arjuna.coordinator.AbstractRecord_1] - AbstractRecord::AbstractRecord () - crash recovery constructor
     */
...
protected AbstractRecord ()
	...
	if (tsLogger.arjLoggerI18N.debugAllowed())
	{
	    tsLogger.arjLoggerI18N.debug(DebugLevel.CONSTRUCTORS, VisibilityLevel.VIS_PROTECTED,
					 FacilityCode.FAC_ABSTRACT_REC, "com.arjuna.ats.arjuna.coordinator.AbstractRecord_1");
	}
    }
