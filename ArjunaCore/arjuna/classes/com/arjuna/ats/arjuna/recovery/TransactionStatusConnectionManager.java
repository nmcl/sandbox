 ...
 * UK.
 *
 * $Id: TransactionStatusConnectionManager.java,v 1.3 2003/08/11 15:04:16 nmcl Exp $
 */
...
public class TransactionStatusConnectionManager
{
	...
	if ( _tscTable.containsKey ( process_id ) )
		...
		TransactionStatusConnector
		    tsc = (TransactionStatusConnector) _tscTable.get( process_id ) ;
	...
	if ( status == ActionStatus.INVALID )
	{
	    try
	    {
		int osState = _objStore.currentState( tranUid, transactionType ) ;
		...
		{
		    tsLogger.arjLoggerI18N.warn("com.arjuna.ats.arjuna.recovery.TransactionStatusConnectionManager_1", new Object[]{ex});
		}
	    }
	...
	while ( tsmiEnum.hasMoreElements() )
	{
	    ...
	    {
		TransactionStatusConnector
		    tsc = new TransactionStatusConnector ( process_id, currentUid ) ;
