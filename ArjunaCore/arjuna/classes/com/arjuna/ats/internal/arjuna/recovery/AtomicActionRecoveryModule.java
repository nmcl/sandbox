 ...
 * UK.
 *
 * $Id: AtomicActionRecoveryModule.java,v 1.3 2003/08/11 15:04:20 nmcl Exp $
 */
...
public class AtomicActionRecoveryModule implements RecoveryModule
{
   ...
   public void periodicWorkFirstPass()
   {
	  ...
	  //	  _transactionStatusConnectionMgr.update() ;

	  AtomicActions = _transactionStore.allObjUids( _transactionType,
                                                       aa_uids );
   ...
   private void doRecoverTransaction( Uid recoverUid )
   {
      ...
      int theStatus = _transactionStatusConnectionMgr.getTransactionStatus
                         ( _transactionType, recoverUid ) ;
   ...
   private void processTransactionsStatus()
   {
      ...
      while ( transactionUidEnum.hasMoreElements() )
      {
         ...
         try
         {
            if ( _transactionStore.currentState( currentUid, _transactionType )
                 != ObjectStore.OS_UNKNOWN )
            {
