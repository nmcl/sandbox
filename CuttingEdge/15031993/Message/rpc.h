 ...
 *
 * $Id: rpc.h,v 1.14 1993/02/19 11:45:13 nmcl Exp $
 */
...
#define SR_TERM_TO      18000    /* server dies after 3 mins expiration
                                  * from the time it is asked to terminate
                                  */
#define CL_TERM_TO       1000    /* (10 secs) for client to receive ack from server */
#define SERVERTOUT       1000    /* (10 secs) For a server to get ack from
				     another server - increased for debugging
				     tests */
#define RETRYT_OUT       600    /* (6 secs) For client to send retries, expected by server */

#ifdef DEBUG
#define GWTIMEOUT       8000    /* timeout for getting server */
#else
#define GWTIMEOUT       4000    /* timeout for getting server */
#endif

#define SYNCH_TOUT       5000    /* For client to get response from server - reduce later */
#define ASYNCH_TOUT     20000    /* For client to get response from server via manager */
...
#define ROUNDTRIP_TIME    20     /* Approx roundtrip time in msecs for ipc */

/******************************/
/* Retry values for protocols */
/******************************/

#define HK_MAXRETRY     3      /* for housekeeper to send request to manager  */
#define HK_REPLYRETRY   5      /* for manager to send crash count to housekeeper */
#define CL_TERM_RETRY   3      /* for client to send terminate message to server */
#define GWRETRY         1      /* Number of retries internal to initiate() */
                               /* change to 10 at least */
#define SYNCH_RETRY     1      /* For client to get response from deaf server - increase later? */
#define ASYNCH_RETRY    2      /* For client to get response from deaf server via manager - increase later? */
...
#ifndef MANAGER_ADDR
#define MANAGER_ADDR 3988            /* Well known manager port number */
#endif
