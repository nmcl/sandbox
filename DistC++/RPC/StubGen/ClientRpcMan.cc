 ...
 * UK.
 *
 * $Id: ClientRpcMan.cc,v 1.3 1998/11/20 08:28:52 nmcl Exp $
 */

static const char RCSid[] = "$Id: ClientRpcMan.cc,v 1.3 1998/11/20 08:28:52 nmcl Exp $";
...
public:
    ...
    ~RpcImpl ();
    RPC_Status call (const Uid&, Int32, RpcBuffer&, Int32&, RpcBuffer&);
...
ClientRpcManager::ClientRpcManager ( ClientRpc *clientHandle, int byDefault, const Uid& objID )
				   : abortHandler(0),
				     deleteMe(byDefault),
				     theHandle(0)
{
...

{
    if (theHandle)
    {
	Uid objectID(NIL_UID);

	theHandle->getRpcHandle()->control().getObjectUid(objectID);
	
	return theHandle->call(objectID, opcode, call, errcode, result);
    }
    else
	return OPER_NOTDONE;
}
...
RPC_Status RpcImpl::call ( const Uid& objID, Int32  opcode, RpcBuffer& call,
			   Int32& errcode, RpcBuffer& result)

{
    if (initiated && (rpcHandle))
	return rpcHandle->call(objID, opcode, call, errcode, result);
    else
	return OPER_NOTDONE;
}
