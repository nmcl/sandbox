 ...
 * UK.
 *
 * $Id: ClassT.cc,v 1.4 1998/11/20 08:28:53 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: ClassT.cc,v 1.4 1998/11/20 08:28:53 nmcl Exp $";
...
Boolean ClassType::produceAllStubs ( ostream& masterHdr )
{
	...
	classHeader << "#include <Template/ServerObjectManager.h>\n";
	classHeader << "#include <Thread/ThreadMutex.h>\n";
	classHeader << "#include <Template/TSHashList.h>\n";
...
void ClassType::printClientSpecials ( ostream& s )
{
    ...
    s << "\n";
    printTemplatePrefix(s);
    s << "ClientRpcManager& " << typeName(CLIENT_DECL) << "::rpcManager ()\n";
    s << "{\n";
    s << "return _clientManager;\n";
    s << "}\n";
}
...
void ClassType::printServerSpecials ( ostream& s )
{
    ...
    s << classServerName << " () :\n";
    s << "\tobjectList(new ThreadSafeHashList< ServerObjectManager<" << typeName(SERVERIMPLEMENTATION_DECL) << "> >(21)),\n";
    s << "\tmutex(ThreadMutex::create())\n";
    s << "{}\n";
	...
	s << "if (objectList != 0)\n";
	s << "{\n";
	s << "\tServerObjectManager* obj = objectList->orderedPop();\n\n";
	s << "\twhile (obj != 0)\n";
	s << "\t{\n";
	s << "\t\tdelete obj;\n";
	s << "\t\tobj = objectList->orderedPop();\n";
	s << "\t}\n\n";
	s << "\tdelete objectList;\n";
	s << "}\n";
    }
    ...
    s << "for (;;)\n{\n";
    s << "RpcBuffer* work = new RpcBuffer;\n\n";

    s << "ServerConnection* connection = ServerRPC.getWork(opcode, objectID, *work);\n\n";

    s << "ServerObjectData<" << typeName(SERVER_DECL)<< ">* data = new ServerObjectData<" << typeName(SERVER_DECL)<< ">(this, work, opcode, objectID, connection);\n";
    s << "Thread* t = new Thread(threadBody, data);\n";
    s << "t->resume();\n\n";
    ...
    /* Now the thread body code, which does the real work. */
    
    s << "\n";
    printTemplatePrefix(s);
    s << "void* " << typeName(SERVER_DECL) << "::threadBody ( void* param )\n";
    s << "{\n";
    s << "ServerObjectData<" << typeName(SERVER_DECL)<< ">* data = (ServerObjectData<" << typeName(SERVER_DECL)<< ">*)param;\n";
    s << typeName(SERVER_DECL) << "* server = 0;\n";
    s << "RpcBuffer result;\n";    
    s << typeName(SERVERIMPLEMENTATION_DECL) << "* theRealObject = 0;\n";
    s << "ServerObjectManager<" << typeName(SERVERIMPLEMENTATION_DECL) << ">* objManager = 0;\n";
    s << "Int32 errcode;\n\n";
    
    s << "if (data == 0)\n";
    s << "return 0;\n";
    s << "else\n";
    s << "server = data->object;\n\n";

    s << "if (server->objectList && (data->objectID != NIL_UID))\n";
    s << "{\n";
    s << "objManager = server->objectList->lookFor(data->objectID);\n";
    s << "if (objManager != 0)\n";
    s << "theRealObject = objManager->_theRealObject;\n";
    s << "}\n\n";

    s << "Thread::yield();\n\n";

    s << "if ((theRealObject != 0) || (data->objectID == NIL_UID))\n";
    s << "errcode = server->dispatchToClass(theRealObject, data->opcode, data->objectID, *(data->work), result);\n";
    s << "else\n";
    s << "errcode = DISPATCH_ERROR;\n\n";
    
    s << "data->connection->sendResult(errcode, result);\n\n";
    
    s << "delete data;\n\n";
    
    s << "Thread* curr = Thread::current();\n";
    s << "if (curr)\n";
    s << "delete curr;\n\n";
    ...
    s << "Int32 funcCode, const Uid& objectID, RpcBuffer& work, RpcBuffer& result)\n{\n";
    s << "Int32 classCode;\n\nwork >> classCode;\n\n";
    s << "switch (classCode)\n{\n";

    s << "case -1:\n";
    s << "return dispatchToOper(theObject, funcCode, objectID, work, result);\n";
	...
	while (blist)
	{
	    ...
	    s << "return ";
	    s << cl->typeName(SERVER_DECL) << "::dispatchToClass(theObject, funcCode, objectID, work, result);\n";
	    blist = blist->getNext();
	}
    ...
    s << " * theObject, Int32 funcCode, const Uid& objectID, RpcBuffer& work, RpcBuffer& result)\n{\n";
    s << "switch (funcCode)\n{\n";
	...
	errorLocation = decl->getLoc();
		...
		if (!funcptr->isPure())
		{
		    ...
		    s << "return ";
		    s << funcptr->simpleTypeName(SERVER_DECL) << "(theObject, objectID, work, result);\n";
...
void ClassType::printServerTypedef ( ostream& s )
{
	...
	s << "*, Int32, const Uid&, RpcBuffer&, RpcBuffer&);\n\n";
	...
	s << "\n\tThreadSafeHashList< ServerObjectManager<" << typeName(SERVERIMPLEMENTATION_DECL) << "> >* objectList;\n";
	s << "\tThreadMutex* mutex;\n";
	s << "}";
    }
}
...
Boolean ClassType::produceServerCode ( ostream& codeStream )
{
	...
	if (classStubLoc != STUB_NOREMOTE)
	{
	    codeStream << "#include <Thread/Thread.h>\n";
	    codeStream << "#include <Template/ServerObjectData.h>\n";
