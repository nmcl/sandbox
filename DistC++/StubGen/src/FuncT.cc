 ...
 * UK.
 *
 * $Id: FuncT.cc,v 1.4 1998/02/06 12:09:50 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: FuncT.cc,v 1.4 1998/02/06 12:09:50 nmcl Exp $";
				    ...
				    Boolean noConst)
{
	...
	s << "(" << funcIsmemberOf->typeName(SERVERIMPLEMENTATION_DECL) << "*, const Uid&, RpcBuffer&, RpcBuffer&)";
	break;
    case IDL_DECL:
	if (funcIsmemberOf)
	{
...
Boolean FctType::printClientCode ( ostream& s )
{
    ...
    /*
     * If a constructor then unpack the object identity and
     * pass it to the client manager for future use.
     */
    
    if (ftype == CONSTRUCTOR)
    {
	s << "Uid objID(NIL_UID);\n\n";
	s << "_replyBuffer_ >> objID;\n";
	s << "_clientManager.getRpcHandle()->control().setObjectUid(objID);\n";
    }
...
Boolean FctType::printServerCode ( ostream& s )
{
    ...
    s << "* _theObject_, const Uid& objectID, RpcBuffer& _work_, RpcBuffer& _result_)";
    ...
    if ((ftype == CONSTRUCTOR) || (ftype == DESTRUCTOR))
	s << "ServerObjectManager<" << funcIsmemberOf->typeName(SERVERIMPLEMENTATION_DECL) << ">* objManager = 0;\n";
    ...
    {
	s << "if (_theObject_ == 0)\n";
	s << "_errCode_ = DISPATCH_ERROR;\n";
	s << "else\n";
	s << "{\n";
	s << "mutex->lock();\n";
	s << "objManager = objectList->lookFor(objectID);\n\n";
	
	s << "if (--objManager->_constructRefCount_ == 0)\n{\n";
	s << "delete _theObject_;\n";
	s << "_theObject_ = 0;\n";
	s << "objectList->remove(objManager);\n";
	s << "delete objManager;\n";
	s << "}\n\n";
	s << "mutex->unlock();\n";
	s << "}\n";
    } else
    {
	    ...
	    s << "if (objManager && ((objManager->_constructRefCount_ > 0) && "
	      << "((objManager->_constructOpcode_ != " << funcHashval << ") || "
	      << "(objManager->_constructArgs_ != _savedArgs_))))\n";
	    
	    s << "_errCode_ = ALREADY_CONSTRUCTED;\n";
	    s << "else\n{\n";
	    s << "if (objManager && objManager->_constructRefCount_ > 0)\n";
	    s << "{\n_result_ = objManager->_constructResult_;\n";
	    s << "}\n";
	    s << "else\n{\n";
	    s << "_constructedHere_ = 1;\n";
	    s << "_theObject_ = new " << funcIsmemberOf->typeName(SERVERIMPLEMENTATION_DECL);
        } else
        {                               /* normal operation */
            if (hasNonVoidResult)
	    {
	    ...
	    /*
	     * Generate code with explicit class type in so we always
	     * call the right version of a method, even if it's virtual.
	     * This is ok since we pass the hierarchy level information
	     * explicitly anyway, so if we get here in the stub hierarchy
	     * we need to call the corresponding method in the real
	     * object hierarchy.
	     */
	    
            s << "_theObject_->" << funcIsmemberOf->typeName(SERVERIMPLEMENTATION_DECL) << "::" << funcName;
        }
	...
	if (ftype == CONSTRUCTOR)
	{
	    ...
	    s << "objManager = new ServerObjectManager<" << funcIsmemberOf->typeName(SERVERIMPLEMENTATION_DECL) << ">(_theObject_);\n";
	    s << "objectList->add(objManager);\n\n";
	    s << "_result_ << objManager->get_uid();\n";
	}
	...
	s << "objManager->_constructRefCount_++;\n";
	s << "if (objManager->_constructRefCount_ == 1)\n{\n";
	s << "objManager->_constructResult_ = _result_;\n";
	s << "objManager->_constructArgs_ = _savedArgs_;\n";
	s << "objManager->_constructOpcode_ = " << funcHashval << ";\n";
	s << "}\n}\n\n";
