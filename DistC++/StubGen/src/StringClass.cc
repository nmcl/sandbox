 ...
 * UK.
 *
 * $Id: StringClass.cc,v 1.1 1997/09/25 15:31:21 nmcl Exp $
 */
...
// static const char RCSid[] = "$Id: StringClass.cc,v 1.1 1997/09/25 15:31:21 nmcl Exp $";
...
/*
 * Remove particular chars from a string. Return new string as the result
 * Since strtok modifies the string we cannot use the copy ctor. Instead
 * we force use of another ctor that clones the string
 */

String String::removeChars ( const String& charsOut ) const
{
    String temp = String(sp->s);	/* DO NOT USE copy ctor */
    String newS;
    char *c = temp;			/* pointer to pass on */
    char *p = ::strtok(c,charsOut);
...
String String::strpbrk ( const char *lookfor ) const
{
    register char *cp = ::strpbrk(sp->s, lookfor);
...
#ifdef NO_INLINES
#  define STRINGCLASS_CC_
#  include "StringClass.n"
#  undef STRINGCLASS_CC_
#endif
