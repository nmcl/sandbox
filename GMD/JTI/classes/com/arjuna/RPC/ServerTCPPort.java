 ...
 * UK.
 *
 * $Id: ServerTCPPort.java,v 1.1 1998/04/08 10:49:44 nmcl Exp $
 */
...
public ServerTCPPort (PortAddress addr) throws IOException
    {
	if (!addr.valid())
	...
	_valid = true;
	_isConnected = false;
	_acceptingSocket = new ServerSocket(addr.portNumber());
    }
...
public synchronized void sendMessage (OutputBuffer[] buff) throws IOException
    {
	if (!_isConnected)
	    throw(new IOException("ServerTCPPort not connected."));
...
public synchronized PortAddress acceptConnection () throws IOException
    {
	if (_isConnected)
	    throw(new IOException("Socket already connected."));

	_sock = _acceptingSocket.accept();
	_myOutputStream = _sock.getOutputStream();
	_myInputStream = _sock.getInputStream();
	_isConnected = true;

	int port = _sock.getPort();
	InetAddress inet = _sock.getInetAddress();
	String host = inet.getHostName();

	return new PortAddress(host, port);
    }

public synchronized void breakConnection () throws IOException
    {
	if (!_isConnected)
...
private ServerSocket _acceptingSocket;
