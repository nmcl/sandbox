 ...
 * UK.
 *
 * $Id: Posix_Thread.cc,v 1.11 1998/08/29 10:36:42 nmcl Exp $
 */

#if !defined(PTHREAD_DRAFT_6) && !defined(OS_SIGNAL_H_)
#  include <os/signal.h>
#endif
			 ...
			 isSuspending(FALSE)
{
    ...
    pthread_cond_init(sleepCondition, &cattr);
    pthread_cond_init(waiters, &cattr);
#endif
		 ...
		 _cleanupList(NULL)		 
{
    ...
    param.prio = Thread::defaultPriority;
#else    
...
#else
    (void) pthread_getschedattr(pthread_self(), &_theThread->attribute);
    (void) pthread_attr_setprio(&_theThread->attribute, Thread::defaultPriority);
    (void) pthread_setschedattr(pthread_self(), _theThread->attribute);
#endif
		 ...
		 _cleanupList(NULL)		 
{
    ...
    pthread_attr_setschedparam(&_theThread->attribute, &param);
#else
    pthread_attr_setprio(&_theThread->attribute, prio);
#endif    

    pthread_attr_setstacksize(&_theThread->attribute, (unsigned int) stackSize);
...
Thread::~Thread ()
{
    ...
    /*
     * Make sure the thread really has died before we
     * try to reclaim storage. This prevents a race condition between
     * the thread broadcasting on its condition and another thread
     * deleting its storage.
     */
...
/*
 * Would like to be able to reference count in wait to make sure the
 * thread isn't deleted while we are waiting for it. However, currently
 * we can't since the thread destructor calls wait if the thread object is
 * not deleted by its owner, and this results in a recursive call on the
 * Resource base class.
 */
...
#if defined(PTHREAD_DRAFT_8) || defined(PTHREAD_SUN)
    p1 = pthread_getspecific(ThreadData::threadKey);
#else
    error = pthread_getspecific(ThreadData::threadKey, &p1);
#endif

    Thread* aThread = (Thread*) p1;
    
    /*
     * Draft 6 pthreads enables getspecific to return an error if the
     * thread did not have data associated with it. However, since then
     * the signature has changed, and this is no longer possible - it is
     * not possible to distinguish between a NULL value and nothing present.
     * However, we always try to associate data with a key, so we can
     * guarantee that the absence of a value means the thread must be on
     * the overflow list. So, find it the hard way!
     */
    
    if ((aThread == 0) && (error == 0))
    {
	/*
...
#endif    
#else
	pthread_attr_getprio(&_theThread->attribute);
	return _theThread->attribute.prio;
#endif
    }
    else
...
#else
	pthread_getschedattr(_toControl->_theThread->sysId, &_toControl->_theThread->attribute);
	pthread_attr_setprio(&_toControl->_theThread->attribute, prio);
	pthread_setschedattr(_toControl->_theThread->sysId, _toControl->_theThread->attribute);
#endif
	return TRUE;
    }
    else
