 ...
 * UK.
 *
 * $Id: PC_Thread.cc,v 1.9 1998/08/29 10:36:41 nmcl Exp $
 */
...
#  include <Common/Thread/PC_Thread/PC_Thread.h>
#endif
...
const unsigned long Thread::defaultStackSize = PTHREAD_STACK_MIN;
		 ...
		 _cleanupList(NULL)		 
{
    ...
    pthread_setspecific(ThreadData::threadKey, (any_t) this);
    pthread_setprio_np(pthread_self(), Thread::defaultPriority);
...
Thread* Thread::current ()
{
    ...
    any_t p1 = (any_t) 0;
    int error = pthread_getspecific(ThreadData::threadKey, &p1);
...
/*
 * PCthreads always start running immediately. If the user wanted them created
...
{
    if (status() != Thread::TERMINATED)
    {
	sched_param param;

	if (pthread_attr_getschedparam(&_theThread->attribute, &param) == 0)
...
{
    if (_toControl->status() != Thread::TERMINATED)
    {
	sched_param param;
	param.sched_priority = prio;
    
	if (pthread_attr_setschedparam(&_toControl->_theThread->attribute, &param) == 0)
	    return TRUE;
    }
