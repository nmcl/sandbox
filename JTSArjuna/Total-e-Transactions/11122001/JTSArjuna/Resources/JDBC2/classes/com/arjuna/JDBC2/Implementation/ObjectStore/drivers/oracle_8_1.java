 ...
 * UK.  
 *
 * $Id: oracle_8_1.java,v 1.1.2.2.30.1.2.1 2002/06/17 12:46:53 nmcl Exp $
 */
...
public class oracle_8_1 implements JDBCImple
{
...
public oracle_8_1 ()
	...
	_preparedStatements = new PreparedStatement[oracle_8_1.STATEMENT_SIZE];

	for (int i = 0; i < oracle_8_1.STATEMENT_SIZE; i++)
...
public boolean initialise (Connection conn, String tableName, boolean dropTable)
	...
	try
	{
	    ...
	    stmt.executeUpdate("CREATE TABLE "+_jdbcTableName+" (StateType INTEGER, TypeName VARCHAR(1024),UidString VARCHAR(255),ObjectState BLOB)");
	...
	try
	{
		...
		catch (NullPointerException ex)
		{
		...
		catch (SQLException ex)
 		{
 		    ...
 		    if (ex.getErrorCode() == 17011) // exhausted result set
 		    {
 		    }
 		    else
 			ex.printStackTrace();
