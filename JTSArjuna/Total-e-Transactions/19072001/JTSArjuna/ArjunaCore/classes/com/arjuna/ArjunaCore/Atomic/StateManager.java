 ...
 * UK.
 *
 * $Id: StateManager.java,v 1.1.4.1.2.4.2.2.2.2.4.5 2001/01/24 13:42:17 nmcl Exp $
 */
 ...
 * @version $Id: StateManager.java,v 1.1.4.1.2.4.2.2.2.2.4.5 2001/01/24 13:42:17 nmcl Exp $
 * @since JTS 1.0.
 */
...
public boolean save_state (OutputObjectState os, int ot)
	...
	if (ot == ObjectType.ANDPERSISTENT)
	{
		...
		if (action == null)
		    ...
		    packHeader(os, action.getSavingUid(), Utility.getProcessUid());
	    }
	    catch (IOException e)
	    {
		return false;
...
public void finalize () throws Throwable
	...
	if (currentStatus == ObjectStatus.ACTIVE_NEW)
	{
	    ...
	    {
		ErrorStream.warning().println("StateManager::terminate() should be invoked in every destructor");
...
public synchronized boolean activate (String rootName)
	...
	if (DebugController.enabled())
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PUBLIC,
						 FacilityCode.FAC_STATE_MAN, "StateManager::activate( "+((rootName != null) ? rootName : "null")+") for object-id "
						 +objectUid);
	}
	    ...
	    (currentStatus == ObjectStatus.PASSIVE_NEW))
	{
		...
		if (loadObjectState())
		{
		    ...
		    {
			ErrorStream.warning().println("Activate of object with id = "+
...
public synchronized boolean deactivate (String rootName, boolean commit)
	...
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PUBLIC,
						 FacilityCode.FAC_STATE_MAN, "StateManager::deactivate("+((rootName != null) ? rootName : "null")+", "+commit+") for object-id "
						 +objectUid);
	}
	...
	if ((currentlyActivated  && (myType == ObjectType.ANDPERSISTENT)) || loadObjectState())
	{
		...
		if (save_state(newState, myType))
		{
		    ...
		    {
			ErrorStream.warning().println("StateManager::deactivate - object store error "+e);
			
			result = false;
		    }
		}
		else
		    ErrorStream.warning().println("StateManager::deactivate - save_state error");
	...
	else
	{
	    ...
	    if (DebugController.enabled())
		ErrorStream.warning().println("StateManager::deactivate() called for non-persistent object - ignored");
	}
...
public synchronized boolean destroy ()
	...
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PUBLIC,
						 FacilityCode.FAC_STATE_MAN, "StateManager::destroy for object-id "+objectUid);
	}
	...
	if (objectStore != null)
	{
		...
		if (action.add(dr) != AddOutcome.AR_ADDED)
		{
		    ...
		    ErrorStream.warning().println("StateManager.destroy - failed to add abstract record.");
		}
		else
		    result = true;
	    }
		...
		{
		    ErrorStream.warning().println("StateManager.destroy - caught object store exception: "+e);
		    result = false;
		}
	    }
	}
	else
	{
	    ...
	    if (DebugController.enabled())
		ErrorStream.warning().println("StateManager.destroy - called on non-persistent or new object!");
	}
...
protected void unpackHeader (InputObjectState os, Uid txId,
	...
	try
	{
	    ...
	    {
		ErrorStream.warning().println("StateManager.restore_state - could not find StateManager state in object state!");

		throw new IOException("Invalid object state.");
	    }
	}
	catch (IOException ex)
	{
...
protected void terminate ()
	...
	if (DebugController.enabled())
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PROTECTED,
						 FacilityCode.FAC_STATE_MAN, "StateManager::terminate() for object-id "+get_uid());
	}
...
protected synchronized final boolean modified ()
    {
	if (DebugController.enabled())
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PROTECTED,
	...
	if (currentStatus == ObjectStatus.PASSIVE)
	{
	    ErrorStream.warning().println("StateManager::modified() invocation on an object whose state has not been restored - activating object");
	...
	synchronized (usingActions)
		...
		if (usingActions != null)
		{
			...
			if (action != null)
			{
				...
				if (DebugController.enabled())
				    ErrorStream.warning().println("Delete called on object with uid "
								  +objectUid+" and type \""+type()
								  +"\" within atomic action.");
				...
				if (fromTerminate)
				{
				    ...
				    {
					ErrorStream.warning().println("StateManager.cleanup - could not save_state from terminate!");
...
protected synchronized void setupStore (String rootName)
	...
	{
	    DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PROTECTED,
						 FacilityCode.FAC_STATE_MAN, "StateManager::setupStore ( "+((rootName != null) ? rootName : "null")+" )");
	}
	...
	else
	{
	    ...
	    ErrorStream.warning().println("Attempt to use volatile store.");
	    
	    throw new com.arjuna.ArjunaCommon.Common.FatalError("Volatile store not implemented!");
		
	    //		objectStore = new ObjectStore(ArjunaCoreNames.Implementation_ObjectStore_VolatileStore(), storeRoot);
	}
...
private void parseObjectName ()
	...
	if (objectName != null)
	{
	    try
	    {
		objectUid = objectName.getUidAttribute(ArjunaCoreNames.StateManager_uid());
	    }
	    catch (Exception e)
	    {
		// assume not present
	    ...
	    try
	    {
		myType = (int) objectName.getLongAttribute(ArjunaCoreNames.StateManager_objectType());
	    }
	    catch (Exception e)
	    {
		myType = ObjectType.ANDPERSISTENT;
	    ...
	    try
	    {
		smAttributes.objectModel = (int) objectName.getLongAttribute(ArjunaCoreNames.StateManager_objectModel());
	    }
	    catch (Exception e)
	    {
		// assume not present.
		...
		smAttributes.objectStoreType = objectName.getClassNameAttribute(ArjunaCoreEnvironment.OBJECTSTORE_TYPE);
	    }
	    catch (Exception e)
	    {
		// assume not present.
