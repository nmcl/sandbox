 ...
 * UK.  
 *
 * $Id: ShadowingStore.javatmpl,v 1.1.4.1.2.4.2.2.2.3.4.2.8.1.6.1.24.2 2001/07/19 13:58:19 nmcl Exp $
 */
 ...
 * @version $Id: ShadowingStore.javatmpl,v 1.1.4.1.2.4.2.2.2.3.4.2.8.1.6.1.24.2 2001/07/19 13:58:19 nmcl Exp $
 * @since JTS 1..0
 */
	...
	if (DebugController.enabled())
	{
	    synchronized (DebugController.controller())
		{
		    DebugController.controller().print(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PUBLIC,
						       FacilityCode.FAC_OBJECT_STORE, "ShadowingStore.currentState("+objUid+", "+tName+") - returning ");
			...
			DebugController.controller().println(DebugLevel.FUNCTIONS, VisibilityLevel.VIS_PUBLIC, FacilityCode.FAC_OBJECT_STORE, "");
		    }
		}
	}
...
public boolean commit_state (Uid objUid, String tName) throws ObjectStoreException
	...
	if (!storeValid())
	{
	    ErrorStream.warning().println("ShadowingStore.commit_state - store invalid!");
	...
	if (tName != null)
	{
		...
		if (!result)
		{
		    ErrorStream.warning().println("ShadowStore::commit_state - failed to rename "+shadow+" to "+filename);
		}
		else
		{
...
public boolean hide_state (Uid objUid, String tName) throws ObjectStoreException
	...
	if (storeValid())
	{
	    ...
	    case ObjectStore.OS_COMMITTED:
		{
		    ...
		    {
			ErrorStream.warning().println("ShadowStore::hide_state - failed to rename "+newState+" to "+oldState);
		    }
...
public boolean reveal_state (Uid objUid,
	...
	if (storeValid())
	{
	    ...
	    case ObjectStore.OS_UNCOMMITTED_HIDDEN:
		{
		    ...
		    {
			ErrorStream.warning().println("ShadowStore::reveal_state - failed to rename "+newState+" to "+oldState);
		    }
...
public static ShadowingStore create (Object[] param)
	...
	if (shareStatus != null)
	{
	    ...
	    {
		ErrorStream.warning().println("ShadowingStore.create caught: "+e);
	    }
	}
...
protected InputObjectState read_state (Uid objUid, String tName,
	...
	if (!storeValid())
	{
	    ErrorStream.warning().println("ShadowingStore.read_state - store invalid!");
	...
	if (tName != null)
	{
		...
		if (fd != null)
		{
			...
			if ((buffer != null) && (ifile.read(buffer, 0, imageSize) == imageSize))
			{
			    ...
			    ErrorStream.warning().println("ShadowingStore::read_state() failed");
		    }
		    catch (IOException e)
		    {
			closeAndUnlock(fd, ifile, null);
		    ...
		    {
			ErrorStream.warning().println("ShadowingStore::read_state - unlock or close of "+fname+" failed ");
		    }
		}
	    }
	}
...
protected boolean remove_state (Uid objUid, String name, int ft) throws ObjectStoreException
	...
	if (name != null)
	{
		...
		if (fd != null)
		{
			...
			{
			    ErrorStream.warning().println("ShadowingStore::remove_state() - access problems on "+objUid+" and "+name);

			    if (!fd.exists())
				ErrorStream.warning().println("ShadowingStore::remove_state() - state "+objUid+" does not exist for type "+name);
			}
		    }
		    else
		    {
			if (!fd.delete())
			{
			    ...
			    if (ft == ObjectStore.OS_ORIGINAL)
				ErrorStream.warning().println("ShadowingStore::remove_state() - unlink failed on "+fname);
			}
		    }
		...
		{
		    ErrorStream.warning().println("ShadowingStore.remove_state() - fd error for "+objUid);
		...
		ErrorStream.warning().print("ShadowingStore::remove_state() attempted removal of ");
		...
		ErrorStream.stream().println("state for object with uid "+objUid+", type "+name);
	    }
	}
	else
	{
	    ...
	    ErrorStream.warning().println("ShadowingStore.remove_state - type() operation of object with uid "+objUid+" returns NULL");
	}
...
protected boolean write_state (Uid objUid, String tName,
	...
	if (tName != null)
	{
	    ...
	    if (fd == null)
	    {
		ErrorStream.warning().println("ShadowingStore::write_state() - openAndLock failed for "+fname);
		...
		ErrorStream.warning().println("ShadowingStore::write_state - unlock or close of "+fname+" failed.");
...
protected ShadowingStore (int shareStatus)
	...
	{
	    DebugController.controller().println(DebugLevel.CONSTRUCTORS, VisibilityLevel.VIS_PROTECTED,
					       FacilityCode.FAC_OBJECT_STORE, "ShadowingStore.ShadowingStore()");
	}
    }
...
private synchronized final boolean renameFromTo (File from, File to)
	...
	else
	{
		...
		ErrorStream.warning().println("ShadowingStore.renameFromTo - from "+from+" not present. Possibly renamed by crash recovery.");
	    ...
	    {
		ErrorStream.warning().println("ShadowingStore.renameFromTo - failed to lock: "+to);
