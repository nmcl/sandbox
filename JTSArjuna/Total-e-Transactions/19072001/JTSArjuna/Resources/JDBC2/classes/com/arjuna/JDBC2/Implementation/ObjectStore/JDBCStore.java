 ...
 * UK.  
 *
 * $Id: JDBCStore.java,v 1.1.2.3.2.3.58.1 2001/08/09 10:36:57 nmcl Exp $
 */
...
public String getStoreName ()
    {
	if (storeValid())
	    return JDBCStore._className+":"+_jdbcTableName;
	else
	    return "Invalid";
    }
...
protected JDBCStore (String tableName)
	...
	if (!setupStore())
	{
	    ErrorStream.fatal().println("JDBCStore could not setup store < "+_className+", "+_jdbcTableName+" >");
...
protected boolean write_state (Uid objUid, String tName, OutputObjectState state, int s) throws ObjectStoreException
	...
	if (tName != null)
	{
	    ...
	    if (imageSize > JDBCStore._maxStateSize)
		throw new ObjectStoreException("Object state is too large - maximum size allowed: "+JDBCStore._maxStateSize);
...
protected synchronized boolean setupStore ()
	...
	try
	{
	    ...
	    try
	    {
		_theConnection = _jdbcAccess.getConnection();
	    }
	    catch (SQLException ex)
	    {
...
protected final void initialize ()
	...
	_preparedStatements = new PreparedStatement[JDBCStore.STATEMENT_SIZE];

	for (int i = 0; i < JDBCStore.STATEMENT_SIZE; i++)
	    _preparedStatements[i] = null;
    }
	
protected String              _jdbcTableName;
protected Connection          _theConnection;
