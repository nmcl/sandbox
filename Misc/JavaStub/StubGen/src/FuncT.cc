...
Boolean FctType::freezeType ()
{
    ...
    if ((exceptionSpec == 0) &&
	((GlobalInfo::getOutputStyle() == CPLUSPLUS_STYLE) || (GlobalInfo::getOutputStyle() == JAVA_STYLE)) &&
...
void FctType::declareJavaClientLocals ( ostream& s )
{
    ...
    s << "OutputBuffer _callBuffer_ = new OutputBuffer();\n";
    s << "InputBuffer _replyBuffer_ = new InputBuffer();\n";
    s << "int _rpcStatus_ = OPER_UNKNOWN;\n";
    s << "int _serverStatus_ = OPER_INVOKED_OK;\n\n";

    s << "_callBuffer_.packBuffer(_myHashVal);\n";
...
Boolean FctType::printJavaClientCode ( ostream& s )
{
	...
	s << "_returnedValue_ = _returnedValue_0_0.value();\n";
	s << "\n";
    }
    s << "break;\n";
