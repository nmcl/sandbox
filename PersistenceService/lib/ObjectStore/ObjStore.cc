...
Boolean reference ( SetUpInfo& setupInf, ObjectStore* toRef )
{
    ...
    if (!found)
    {
        cerr << "ObjectStoreSetUp::reference - cannot find objectstore instance." << endl;
	return FALSE;
    }
	       ...
	       ObjectStore*& toDelete )
{
    ...
    if (setupInf._headOfList == 0)
    {
	cerr << "Attempt to destroy object store instance without call on ObjectStore::create" << endl;
    ...
    if (!found)
    {
	cerr << "Attempt to destroy non-existant object store instance" << endl;
	delete toDelete;
	toDelete = 0;
	return;
    }
				   ...
				   const char* locationOfStore )
{
    ...
    cerr << "Cannot find " << name << " object store."
...
Boolean ObjectStore::reference ( ObjectStore* toRef )
{
    ...
    if (toRef == 0)
    {
        cerr << "ObjectStore::reference - call on null object." << endl;
	return FALSE;
    }
    ...
    cerr << "Cannot find " << toRef->type() << " object store."
	 << "\n#include RemoteOSTypes.h or LocalOSTypes.h" << endl;
