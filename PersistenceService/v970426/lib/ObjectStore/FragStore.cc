				      ...
				      const TypeName tName )
{
	...
	if (fd != -1)
	{
		...
		{
		    cerr << "FragmentedStore::hide_state()"
					...
					const TypeName tName )
{
	...
	if (fd != -1)
	{
		...
		if (::write(fd, (char*) sdPtr, writeLen) != writeLen)
		{
		    cerr << "FragmentedStore::reveal_state()"
			 << " write failed for " << objUid << endl;
		    closeAndUnlock(cachePtr);
		} else
		{
					...
					const TypeName tName )
{
	...
	if ((fd != -1) && (sdPtr != 0))
	{
		...
		{
		    cerr << "FragmentedStore::commit_state()"
					   ...
					   ObjectStore::StateType ft )
{
	...
	if (fd != -1)
	{
		...
		{
		    cerr << "Attempted read on shadowed/hidden state for "
			 << objUid << "" << endl;
		...
		if (cachePtr->cachedDictPtr->stateHidden)
		{
		    cerr << "Attempted read on hidden state for "
			 << objUid << "" << endl;
#ifndef GCC_ARRAY_DELETE_BUG
		    delete [] readAheadBuffer;
#else
		...
		else
		{
			...
			cerr <<  "FragmentedStore::read_state() failed\n";
					...
					ObjectStore::StateType ft )
{
	...
	if (storeFd != -1)
	{
		...
		{
		    doWrite = FALSE;
		    cerr << "Attempt to remove shadowed/hidden state for object "
			 << objUid << "" << endl;
		}
		break;
	    case ObjectStore::OS_SHADOW:
	    ...
	    case ObjectStore::OS_INVISIBLE:
		doWrite = FALSE;
		cerr << "Attempt to remove hidden state for object "
		     << objUid << "" << endl;
		break;
	    }
			     ...
			     sizeof(struct StateDictionary))) == sizeof(struct StateDictionary))
		{
		    ...
		    if (::fsync(storeFd) == -1)
		    {
			cerr << "fsync failed for storeFd" << endl;
			return FALSE;
		    }
#endif
		    removeOk = closeAndUnlock(cachePtr);
				       ...
				       ObjectStore::StateType ft )
{
	...
	if ((fd != -1) && (sdPtr != 0))
	{
		...
		if (sdPtr->shadowExists)
		{
		    cerr << "Attempt to write shadow state for already shadowed object "
			 << objUid << endl;
		    closeAndUnlock(cachePtr);
		    return FALSE;
		}
		...
		break;
	    case ObjectStore::OS_INVISIBLE:
		cerr << "Attempt to write hidden state for object "
		     << objUid << endl;
		closeAndUnlock(cachePtr);
		return FALSE;
		break;
		...
		if (doWrite)
		{
		    ...
		    {
			cerr << "FragmentedStore::write_state() "
			...
			if (::fsync(fd) == -1)
			{
			    cerr << "fsync failed for " << objUid << endl;
			    closeAndUnlock(cachePtr);
			    return FALSE;
			}
#endif
			       ...
			       char *readAheadBuffer )
{
	...
	if (cachePtr != 0)
	{
		...
		cerr << "FragmentedStore::openAndLock() - fcntl "
