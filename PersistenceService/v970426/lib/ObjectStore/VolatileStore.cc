...
VolatileStore::~VolatileStore ()
{
    ...
    if (storeValid)
    {
	    ...
	    cerr << "VolatileStore::~VolatileStore - closeAndUnlock failed." << endl;
...
Boolean VolatileStore::openAndLockObjectStore ()
{
	...
	{
	    cerr << "VolatileStore::openAndLockObjectStore - shared segment initialise failed." << endl;
	    storeValid = FALSE;
	}
	else
	    result = TRUE;
...
Boolean VolatileStore::writeStates ()
{
	...
	if (!storeValid)
	{
	    cerr << "VolatileStore::writeStates - write failed" << endl;
	    
	    if (!sharedSeg->isValid())
		cerr << "VolatileStore - invalid shared segment" << endl;
	}
	else
	    result = TRUE;
    }
...
Boolean VolatileStore::readStates ()
{
	...
	else
	{
		...
		if (number_states > 0)
		{
		    ...
		    if (!storeValid)
		    {
			cerr << "VolatileStore::read_data - Could not unpack state" << endl;
		    }
		}
	    }
	}
    ...
    {
	cerr << "VolatileStore::readStates - read failed" << endl; 
	if (!sharedSeg->isValid())
	    cerr << "VolatileStore - invalid shared segment" << endl;
    }
				   ...
				   const ObjectState& state, off_t& off_set)
{
	...
	for (int i = 0; (!found) && (marker != 0); i++)
	{
        ...
        cerr << "VolatileStore - store full." << endl;
					...
					ObjectStore::StateType ft)
{
    ...
    if (!tn)
    {
	cerr << "VolatileStore::read_state - Object with uid "
    ...
    {
	cerr << "VolatileStore::read_state - Attempt to read shadow from non-shadowed store." << endl;
	return 0;
    }
...
Boolean VolatileStore::commit_state (const Uid& objUid, const TypeName tn)
{
    ...
    if (!tn)
    {
	cerr << "VolatileStore::commit_state - object with uid "
	     << objUid << " has no TypeName" << endl;
	return FALSE;
    }
				     ...
				     ObjectStore::StateType ft)
{
    ...
    if (!name)
    {
	cerr << "VolatileStore::remove_state - type() operation of object with uid "
	     << objUid << " returns NULL";
	return FALSE;
    }
				     ...
				     ObjectStore::StateType ft )
{
    ...
    if (!tn)
    {
	cerr << "VolatileStore::write_state - Object with uid "
	     << objUid << " has no TypeName\n";
	return FALSE;
    }
	...
	{
	    cerr << "VolatileStore::write_state failed" << endl;
...
Boolean VolatileStore::unpack (Buffer& buff)
{
	...
	if (res)
	{
		...
		{
		    cerr << "VolatileStore - unpack failed" << endl;
		    if (!sharedSeg->isValid())
			cerr << "VolatileStore - invalid shared segment" << endl;
		    unlockObjectStore();
		    storeValid = FALSE;
