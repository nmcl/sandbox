 ...
 * UK.
 *
* $Id: ArjunaDBAccess-utils.cc,v 1.23 1997/04/24 15:34:18 snowman Exp $
 */
...
#else
LocalStudentRecord* theRecord = 0;
#endif
...
char** ServerList = 0;
int NumberOfServers = NUMBER_SERVERS;

extern char *RecordLocked;

Boolean rpc_handler (RPC_Status, Int32) 
{
    error_stream << WARNING << "RecordServer rpc handler called." << endl;
...
Boolean checkNumber (const char* studentNumber)
{
    ...
    {
	for (int i = 0; (i < STUDENTNUMBERSIZE) && result; i++)
	{
    ...
    if (!result)
    {
	error_stream << WARNING << "checkNumber received invalid student number: " << studentNumber << endl;
    }
...
int getServer (const char* stunumber, Uid& id)
{
    ...
    {
	reportf("System has less than minimum number of allowed servers.");
	
	return 25;
    }
    
    if (theRecord != 0)  // we already have a record server!
    {
	...
	if (clientRpc->setHost(hostName) != OPER_DONE)
	{
	    delete clientRpc;
	    clientRpc = 0;
	    return 24;
	...
	if (result == DONE)
	{
	    cout << "Created server on machine " << hostName << " for number " << stunumber << endl;
	    return 0;
	}
	...
	if (virtualFailure)
	{
	    ...
	    error_stream << WARNING << "Host " << hostName << " failed for student "
		         << stunumber << " with " << result << endl;
	
	if (result != UNKNOWN)
	{
	    /*
	     * Treat an operation failure as a virtual failure, and get the user
	     * to try again.
	     */

	    error_stream << WARNING << "The server constructor failed." << endl;
	    return 31;
	}
...
Boolean createIndex (int& error)
{
	...
	counter = new NewStudentRecordNumber(res, counterId);
	...
	error_stream << WARNING << "getCounter host " << ServerList[i] << " failed." << endl;
	reportf("getCounter could not create atomic index.\n");	
    }

    error_stream << WARNING << "getCounter complete host failure." << endl;
...
int getCounter (int& error)
{
	...
	if (B.End() != COMMITTED)
	{
	    error_stream << WARNING << "getCounter action commit failed." << endl;
	    error = 28;
	}
    }
    else
    {
	error_stream << WARNING << "getCounter could not get index number." << endl;
	B.Abort();
	counterValue = -1;
	error = 27;
    }
...
void initArjuna ()
{
    ...
    if (!res)
    {
	error_stream << WARNING << "Error - no replica configuration file found!" << endl;
	abort();
    }
    else
    {
	...
	{
	    error_stream << WARNING << "Error - not enough replicas specified!" << endl;
	    abort();
	}
    }
...
{
    /*
     * Must not be doing any other work.
     */
...
int DBFind( char* StudNumb, char* Reply )
{
#ifdef LOCAL_ONLY
    /*
    ...
    theRecord = new LocalStudentRecord(result, *tempId);
    ...
    theRecord->fetchRecord(result, buff);

    if (result != DONE)  // lock conflict or empty?
    {
        if (result == EMPTY)
	{
...
int DBFetch (char* StudNumb, char* Reply, Boolean Lock)
{
    ...
    char theNumber[STUDENTNUMBERSIZE+1];
    ::memset(theNumber, '\0', STUDENTNUMBERSIZE+1);
    ::memcpy(theNumber, StudNumb, STUDENTNUMBERSIZE);
    
    cout << "Process " << getpid() << " working for student number " << theNumber << endl;
...
#ifndef LOCAL_ONLY
    value = getServer(theNumber, id);
#else

    if (id != NIL_UID)
        theRecord = new LocalStudentRecord(result, id);
    else
    {
        theRecord = new LocalStudentRecord(result);
	id = theRecord->get_uid();
    }
    ...
    theRecord->fetchExclusiveRecord(result, buff);
    ...
    {
	::memcpy(RecordLocked, theNumber, STUDENTNUMBERSIZE);
	...
	if (buff.unpack(ptr, length))
	{
	    ...
	    ::memcpy(tmpReply, Reply, 9);
	    
	    reportf("Opened record: %s\n", tmpReply);
	    
            reportf("Opened information: %s\n", &(Reply[9]));
	}
	else
	{
	    AbortAndCleanUp();	    
...
int DBStore (char* NewData)
{
	...
	NewData[modindex] = '*';
	    ...
	    else
	    {
		    ...
		    ::memcpy(tmpData, NewData, 9);
	    
		    reportf("Saved record: %s", tmpData);
		    
                    reportf("Saved information: %s", &(NewData[9]));
	    	}
	    }
    	}
    }
 ...
 * we must create a new student id and assign the mapping to Uid in
 * the overflowKeyMap.
 */
...
int DBNew (char* StudNumb, char* Reply)
{
    ...
    if (::memcmp(theNumber, CREATENEWPATTERN, STUDENTNUMBERSIZE) == 0)
    {
	/*
	 * All 0s.
	 */
	...
	::memcpy(NewKey, theNumber, STUDENTNUMBERSIZE);
