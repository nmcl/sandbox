/*
 * des: duplicate the NBS Data Encryption Standard in software. usage: des
 * <file> prompts for the password If the filename ends in ".n" it will be
 * decrypted with the key; otherwise it will be encrypted.
 * 
 * Permutation algorithm: The permutation is defined by its effect on each of
 * the 16 nibbles of the 64-bit input.  For each nibble we give an 8-byte bit
 * array that has the bits in the input nibble distributed correctly.  The
 * complete permutation involves ORing the 16 sets of 8 bytes designated by
 * the 16 input nibbles.  Uses 16*16*8 = 2K bytes of storage for each 64-bit
 * permutation.  32-bit permutations (P) and expansion (E) are done
 * similarly, but using bytes instead of nibbles. Should be able to use long
 * ints, adding the masks, at a later pass.  Tradeoff: can speed 64-bit perms
 * up at cost of slowing down expansion or contraction operations by using 8K
 * tables here and decreasing the size of the other tables. The compressions
 * are pre-computed in 12-bit chunks, combining 2 of the 6->4 bit
 * compressions. The key schedule is also precomputed. Compile with VALIDATE
 * defined to run the NBS validation suite.
 * 
 * Jim Gillogly, May 1977 Modified 8/84 by Jim Gillogly and Lauren Weinstein to
 * compile with post-1977 C compilers and systems
 * 
 * This program is now officially in the public domain, and is available for any
 * non-profit use as long as the authorship line is retained.
 */

#include <stdio.h>

char            iperm[16][16][8], fperm[16][16][8];	/* inital and final
							 * permutations */
...
/*
 * char ei[] = {	32,  1,  2,  3,  4,  5, 4,  5,  6,  7,  8,  9, 8,  9,
...
void kinit(char* key)
{
	...
	for (j = 0; j < 56; j++)/* convert pc1 to bits of key   */
	{
		...
		m = l & 07;	/* find bit			 */
		pc1m[j] = (key[l >> 3] &	/* find which key byte l is
						 * in  */
	...
	{
		for (j = 0; j < 56; j++)	/* rotate pc1 the right
						 * amount  */
			pcr[j] = pc1m[(l = j + totrot[i]) < (j < 28 ? 28 : 56) ? l : l - 28];
		/* rotate left and right halves independently   */
		for (j = 0; j < 48; j++)	/* select bits individually	 */
			if (pcr[pc2[j] - 1])	/* check bit that goes to
						 * kn[j] */
			{
				l = j & 07;
				kn[i][j >> 3] |= bytebit[l];
	...
	for (i = 0; i < 4; i++)	/* each input byte position	 */
		for (j = 0; j < 256; j++)	/* all possible input bytes	 */
			for (k = 0; k < 4; k++)	/* each byte of the mask	 */
				p32[i][j][k] = 0;	/* clear permutation
							 * array	 */
	for (i = 0; i < 4; i++)	/* each input byte position	 */
		for (j = 0; j < 256; j++)	/* each possible input byte	 */
			for (k = 0; k < 32; k++)	/* each output bit
							 * position	 */
			{
				l = p32i[k] - 1;	/* invert this bit
							 * (0-31)	 */
				if ((l >> 3) != i)	/* does it come from
							 * input posn? */
					continue;	/* if not, bit k is 0		 */
				if (!(j & bytebit[l & 07]))
					continue;	/* any such bit in
							 * input?	 */
	...
	for (i = 0; i < 16; i++)/* each input nibble position   */
		for (j = 0; j < 16; j++)	/* all possible input nibbles   */
			for (k = 0; k < 8; k++)	/* each byte of the mask	 */
				perm[i][j][k] = 0;	/* clear permutation
							 * array	 */
	for (i = 0; i < 16; i++)/* each input nibble position   */
		for (j = 0; j < 16; j++)	/* each possible input nibble   */
			for (k = 0; k < 64; k++)	/* each output bit
							 * position	 */
			{
				l = p[k] - 1;	/* where does this bit come
						 * from */
				if ((l >> 2) != i)	/* does it come from
							 * input posn? */
					continue;	/* if not, bit k is 0		 */
				if (!(j & nibblebit[l & 3]))
					continue;	/* any such bit in
							 * input?	 */
				m = k & 07;	/* which bit is this in the
						 * byte */
...
void strongk(char* in, char* out)
{
	...
	while (*ci != 0)
	{
		...
		desinit(out);	/* set up tables for DES	 */
		n = 0;
		co = out;
		for (; *ci != 0 && n < 8; ci++)
		{
...
int par(char c)
{
	return ((zpartab[(c >> 4) & 017] + zpartab[c & 017]) % 2 == 0);
}
...
void flipbytes(char* in, char* out, int len)
{
	char            c;
...
void flipwords(Unspecified* in, Unspecified* out, int len)
{
	short int       c;
...
static char     nullvec[] = {0, 0, 0, 0, 0, 0, 0, 0};

void decbcc(char* encr, char* plain, int len)
{				/* decryption algorithm for chained block
...
void encbcc(char* plain, char* encr, int len)
{				/* encryption algorithm for chained block
				 * encryption with checksum */
	int             l;
	char           *e, *p, *k, acc[8];
