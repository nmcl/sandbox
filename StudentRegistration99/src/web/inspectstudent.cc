 ...
 * UK.
 *
 * $Id: inspectstudent.cc,v 1.4 1996/08/29 12:34:17 arjuna Exp $
 */
...
const char* shellName = "FindStudent ";

int inspect (entry entries[], int number)
{
    char* id = 0;
...
int main (int, char **)
{
    ...
    {
	result = inspect(entries, m+1);
    
	if (result != 0)
	{
	    printf("<BODY BGCOLOR=#FFFFFF>");
	    printf("<FONT COLOR=#FF0000>Error number<B> %d </B>occurred when attempting to inspect student.", result);

	    if (result == -1)
		printf("<P>You must give a valid student number.%c", 10);
	    else
		printf("%c", 10);
