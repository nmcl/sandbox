 ...
 * UK.
 *
 * $Id: killuser.cc,v 1.4 1996/08/29 12:34:18 arjuna Exp $
 */
...
int main (int, char **)
{
	...
	{
	    printf("<BODY BGCOLOR=#FFFFFF>");
	    printf("<FONT COLOR=#FF0000>Error number<B> %d </B>occurred when attempting to kill user.", result);

	    if (result == -1)
		printf("<P>You must give a valid user identifier and machine name.%c", 10);
	    else
		printf("%c", 10);
