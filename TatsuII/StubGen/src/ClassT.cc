 ...
 * UK.
 *
 * $Id: ClassT.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: ClassT.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $";
...
static const String arjunaiiclientpref("Client");
		       ...
		       warningsSupressed(FALSE)
{
    ...
    if (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE)
	classClientName = arjunaiiclientpref + name;
}
...
Boolean ClassType::freezeType ()
{
	...
	if (classStubLoc == STUB_REMOTE)
	{
		...
		classClientName = arjunaiiclientpref + className;
...
{  
	...
	case ARJUNAII_SERVER_DECL:
	    return className;
	case ARJUNAII_INTF_DECL:
	    return classIntfName;
	case ARJUNAII_CLIENT_DECL:
	case NORMAL_DECL:
	default:
	    return className;
	}
...
Boolean ClassType::produceAllStubs ( ostream& masterHdr )
{
	...
	Boolean doArjunaII = (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE) ? TRUE : FALSE;
	...
	if (!produceJavaOnly)
	{
	    ...
	    /*
	     * No concept of server main in ArjunaII.
	     */
	...
	if (!produceJavaOnly)
	{
	    ...
	    {
		classHeader << "#include <os/stdlib.h>\n";
		classHeader << "#include <Gandiva.h>\n";
		classHeader << "#include <Core/Resource.h>\n";
		classHeader << "#include <Common/Boolean.h>\n";
		classHeader << "#include <Common/ObjectName.h>\n";
		classHeader << "#include <Interface/Buffer.h>\n";
		classHeader << "#include <Interface/Dispatcher.h>\n";
		classHeader << "#include <Interface/RpcBuffer.h>\n";
		classHeader << "#include <Common/RPC.h>\n";
		classHeader << "#include <Common/AbortHandler.h>\n\n";
	    }
	}
					 ...
					 Boolean full )
{
	...
	if ((classMarshallOpts != STUB_NOMARSHALL) && !doneMarshallDefs)
	{
...
ostream& ClassType::printClassHeader ( ostream& s, DeclStyle ds, Boolean full )
{
    ...
    /* Beware of deliberate supression of virtual mechanism here */
	...
	/*
	 * If ArjunaII, we inherit from the implementation type if
	 * we are doing the client, otherwise we inherit from
	 * the dispatcher base class since it's the server.
	 */
	...
	if (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE)
	{
	    ...
	    if (ds == SERVER_DECL)
		s << "public virtual DispatcherImple";
	    else
	    {
		/*
		 * Eventually we will support derivation from a
		 * the implementation!
		 *
		 * s << "public virtual " << ClassType::simpleTypeName(ARJUNAII_CLIENT_DECL);
		 */

		s << "public virtual Resource";
	    }
	}
    }
...
void ClassType::printArjunaIIClientSpecials ( ostream& s )
{
    BaseClass *blist = vBaseList;
    ...
    printTemplatePrefix(s);
    s << typeName(CLIENT_DECL) << "::" << classClientName;
    s << " ( const ObjectName& manager, const RpcBuffer& phv )";
    ...
    s << "_dispatcher(0), ";
    s << "_clientManager(manager), _myHashVal(phv), _theHandler(0)";
    s << "\n{\n_myHashVal << " << classHashval << " << -1;\n}\n";
...
void ClassType::printArjunaIIClientTypedef (ostream& s)
{
    int i = 0;
    Declaration *decl;
	...
	s << "\tvirtual const ClassName& className () const;\n\n";
	s << "\tstatic Resource* create (const ObjectName&);\n";
	s << "\tResource* control (const ClassName&);\n\n";
	s << "\tvirtual void* castup (const ClassName&) const;\n";
	s << "\tstatic " << classClientName << "* castup (Resource*);\n\n";

	s << "\tvoid rpcAbort (Dispatcher::Outcome, RPC::OperationOutcome);\n";
	s << "\tAbortHandler* setHandler (AbortHandler*);\n\n";
	
	s << "protected:\n";

	s << "\t" << classClientName << " (const ObjectName&, const RpcBuffer&);\n";
	...
	s << "\tDispatcher* _dispatcher;\n";
	s << "\tObjectName _clientManager;\n";
	s << "\tRpcBuffer _myHashVal;\n";
	s << "\tAbortHandler* _theHandler;\n\n";
	
	s << "\tstatic ClassName* _className;\n";
...
void ClassType::printArjunaIIInterfaceTypedef ( ostream& s )
{
    /*
     * To be filled in.
     */
...
void ClassType::printArjunaIIServerSpecials ( ostream& s )
{
    Declaration *decl = 0;
    TypeDescriptor *type = 0;
    int declnum = 0;
    ...
    printTemplatePrefix(s);
    s << typeName(SERVER_DECL) << "::";
    s << classServerName << " (const ObjectName& _objName_) : ";
    s << "theRealObject(0),";
    s << "_constructArgs_(),";
    s << "_constructOpcode_(UNKNOWN),";
    s << "_constructRefCount_(0),";
    s << "_constructResult_(),";
    s << "_objectName_(_name_)";
    s << "\n{}\n";
    ...
    {
	s << "if (theRealObject != 0)\n";
	s << "delete theRealObject; // should unref if derived from resource (and should ref!)\n";
    }
    s << "}\n\n";

    /* Now the dispatch routine. */

    s << "Dispatcher::Outcome " << typeName(SERVER_DECL)
      << "::dispatch (Array<Buffer*>& work, Array<Buffer*>*& result)\n";
    s << "{\n";
    s << "RpcBuffer* myBuff = new RpcBuffer(work[work.getSize()-1]);\n";
    s << "RpcBuffer* myResult = new RpcBuffer;\n";
    s << "RPC::OperationOutcome outcome;\n";
    s << "int opcode;\n\n";

    s << "Resource::ref(myBuff);\n";
    s << "Resource::ref(myResult);\n\n";
    s << "myBuff->unpack(opcode);\n";
    s << "outcome = dispatchToClass(opcode, *myBuff, *myResult);\n\n";
    
    s << "myResult->pack(outcome);\n";
    s << "result = new Array<Buffer*>(1);\n";
    s << "(*result)[0] = myResult;\n\n";

    s << "Resource::unref(myBuff);\n\n";

    s << "switch (outcome)\n";
    s << "{\n";
    s << "\tcase DONE_AND_TERMINATE:\n";
    s << "\t\treturn Dispatcher::DONE_AND_TERMINATE;\n";
    s << "\tcase NOTDONE_AND_TERMINATE:\n";
    s << "\t\treturn Dispatcher::NOTDONE_AND_TERMINATE;\n";
    s << "\tdefault:\n";
    s << "\t\treturn Dispatcher::DONE;\n";
    s << "}\n\n";
    ...
    s << "\n";
    printTemplatePrefix(s);
    s << "RPC::OperationOutcome " << typeName(SERVER_DECL) << "::dispatchToClass ( ";
    s << "int funcCode, RpcBuffer& work, RpcBuffer& result)\n{\n";
    s << "int classCode;\n\nwork >> classCode;\n\n";
    s << "switch (classCode)\n{\n";

    s << "case -1:\n";
    s << "return dispatchToOper(funcCode, work, result);\n";
	...
	while (blist)
	{
	    ...
	    s << "case " << cl->getClassHashval() << ":\n";
	    s << "return ";
	    s << cl->typeName(SERVER_DECL) << "::dispatchToClass(funcCode, work, result);\n";
	    blist = blist->getNext();
	}
    ...
    s << "\n";
    printTemplatePrefix(s);
    s << "RPC::OperationOutcome " << typeName(SERVER_DECL) << "::dispatchToOper ( ";
    s << "int funcCode, RpcBuffer& work, RpcBuffer& result)\n{\n";
    s << "switch (funcCode)\n{\n";
	...
	errorLocation = decl->getLoc();
		...
		if (!funcptr->isPure())
		{
		    ...
		    s << "return ";
		    s << funcptr->simpleTypeName(SERVER_DECL) << "(work, result);\n";
		}
            }
        }
    }
    ...
    /*
     * Now the dispatcher specific methods.
     */

    s << "Dispatcher::Outcome " << typeName(SERVER_DECL) << "::run ()\n";
    s << "{\n\treturn Dispatcher::NOTDONE;\n";
    s << "}\n\n";
    
    s << "const ObjectName& " << typeName(SERVER_DECL) << "::objectName ()\n";
    s << "{\n\treturn ObjectName::invalid();\n";
    s << "}\n\n";

    s << "Buffer* " << typeName(SERVER_DECL) << "::createBuffer ()\n";
    s << "{\n";
    s << "\treturn new Buffer(Gandiva::Implementation::Buffer::QANBO::name());\n";
    s << "}\n\n";

    s << "Resource* " << typeName(SERVER_DECL) << "::create ()\n";
    s << "{\n";
    s << "\treturn new " << typeName(SERVER_DECL) << ";\n";
    s << "}\n\n";

    s << "Resource* " << typeName(SERVER_DECL) << "::create (const ObjectName& name)\n";
    s << "{\n";
    s << "\treturn new " << typeName(SERVER_DECL) << "(name);\n";
    s << "}\n\n";    

    s << "const ObjectName& " << typeName(SERVER_DECL) << "::dispatcherName () const\n";
    s << "{\n\treturn ObjectName::invalid();\n";
    s << "}\n\n";

    s << "const ClassName& " << typeName(SERVER_DECL) << "::name ()\n";
    s << "{\n";
    s << "\tif (_className == (ClassName*) 0)\n";
    s << "\t\t_className = new ClassName(\"" << typeName(SERVER_DECL) << "\");\n";
    s << "\treturn *_className;\n";
    s << "}\n\n";

    s << "const ClassName& " << typeName(SERVER_DECL) << "::className () const\n";
    s << "{\n";
    s << "\treturn ServerObjectDispatcherImple::name();\n";
    s << "}\n\n";

    s << "void* " << typeName(SERVER_DECL) << "::castup (const ClassName& _type) const\n";
    s << "{\n";
    s << "\tif (_type == name())\n";
    s << "\t\treturn (void*) this;\n";
    s << "\telse\n";
    s << "\t\treturn DispatcherImple::castup(_type);\n";
    s << "}\n\n";

    s << typeName(SERVER_DECL) << "* " << typeName(SERVER_DECL) << "::castup (Resource* toCast)\n";
    s << "{\n";
    s << "\tif (toCast == (Resource*) 0)\n";
    s << "\t\treturn NULL;\n";
    s << "\telse\n";
    s << "\t\treturn (" << typeName(SERVER_DECL) << "*) toCast->castup(name());\n";
    s << "}\n\n";

    s << "Resource* " << typeName(SERVER_DECL) << "::control (const ClassName&)\n";
    s << "{\n";
    s << "\treturn (Resource*) 0;\n";
    s << "}\n\n";
}
...
void ClassType::printServerTypedef ( ostream& s )
{
	...
	s << "\tRpcBuffer _constructResult_;\n";
	s << "}";
    }
}

void ClassType::printArjunaIIServerTypedef (ostream& s)
{
    int i = 0;
    Declaration *decl;
	...
	s << "\t" << classServerName << " ();\n";
	s << "\t" << classServerName << " (const ObjectName&);\n";
	s << "\t~" << classServerName << " ();\n\n";

	s << "\tDispatcher::Outcome dispatch (Array<Buffer*>& work, Array<Buffer*>*& result);\n\n";
	s << "\tDispatcher::Outcome run ();\n\n";
	s << "\tBuffer* createBuffer ();\n\n";
	s << "\tconst ObjectName& objectName ();\n";
	s << "\tconst ObjectName& dispatcherName () const;\n\n";

	s << "\tstatic  const ClassName& name ();\n";
	s << "\tvirtual const ClassName& className () const;\n\n";
	s << "\tstatic Resource* create ();\n";
	s << "\tstatic Resource* create (const ObjectName&);\n\n";
	s << "\tResource* control (const ClassName&);\n\n";
	s << "\tvirtual void* castup (const ClassName&) const;\n";
	s << "\tstatic " << classServerName << "* castup (Resource*);\n\n";
	
	s << "protected:\n";
	s << "\tRPC::OperationOutcome dispatchToClass (int, RpcBuffer&, RpcBuffer&);\n";	
	s << "\tRPC::OperationOutcome dispatchToOper (int, RpcBuffer&, RpcBuffer&);\n\n";
	...
	s << "\n\t" << typeName(ARJUNAII_SERVER_DECL) << " *theRealObject;\n\n";
	
	s << "\tRpcBuffer _constructArgs_;\n";
	s << "\tRPC::OperationOutcome _constructOpcode_;\n";
	s << "\tint _constructRefCount_;\n";
	s << "\tRpcBuffer _constructResult_;\n";
	s << "\tObjectName _objectName_;\n\n";
	s << "\tstatic ClassName* _className;\n";
	s << "}";
    }    
}
...
{
    if (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE)
	return produceArjunaIIClientCode(codeStream);
...
Boolean ClassType::produceArjunaIIClientCode ( ostream& codeStream )
{
    if (classFullDecl)
	return classFullDecl->produceArjunaIIClientCode(codeStream);
    else
    {
	Boolean stubOk = TRUE;
	Declaration *decl;
	...
	if (classStubLoc != STUB_NOREMOTE)
	{
	    codeStream << "#include <os/iostream.h>";
	    codeStream << "\n#include \"" << className << GlobalInfo::getClassHdrSuff()
		       << "\"\n\n";

	    codeStream << "ClassName* " << typeName(CLIENT_DECL) << "::_className = NULL;\n";

	    printArjunaIIClientSpecials(codeStream);
	    ...
	    codeStream << "\nvoid " << typeName(CLIENT_DECL) << "::rpcAbort (Dispatcher::Outcome o1, RPC::OperationOutcome o2)\n";
	    codeStream << "{\n";
	    codeStream << "\tif (!_theHandler)\n";
	    codeStream << "#ifndef WIN32\n";
	    codeStream << "\t\tkill(getpid(), SIGQUIT);\n";
	    codeStream << "#else\n";
	    codeStream << "\t\tabort();\n";
	    codeStream << "#endif\n";
	    codeStream << "\telse\n\t\t_theHandler->rpcAbort(o1, o2);\n";
	    codeStream << "}\n\n";
	    codeStream << "AbortHandler* " << typeName(CLIENT_DECL) << "::setHandler (AbortHandler* handler)\n";
	    codeStream << "{\n";
	    codeStream << "\tAbortHandler* ptr = _theHandler;\n\n";
	    codeStream << "\t_theHandler = handler;\n";
	    codeStream << "\treturn ptr;\n";
	    codeStream << "}\n\n";

	    codeStream << "ClassName& " << typeName(CLIENT_DECL) << "::className () const\n";
	    codeStream << "{\n\treturn " << typeName(CLIENT_DECL) << "::name();\n";
	    codeStream << "}\n\n";

	    codeStream << "ClassName& " << typeName(CLIENT_DECL) << "::name ()\n";
	    codeStream << "{\n";
	    codeStream << "\tif (_className == NULL)\n";
	    codeStream << "\t\t_className = new ClassName(\"" << typeName(CLIENT_DECL) << "\");\n";
	    codeStream << "\treturn *_className;\n";
	    codeStream << "}\n\n";

	    codeStream << "Resource* " << typeName(CLIENT_DECL) << "::create (const ObjectName& objName)\n";
	    codeStream << "{\n";
	    codeStream << "\treturn (Resource*) 0;\n";
	    codeStream << "}\n\n";

	    codeStream << "Resource* " << typeName(CLIENT_DECL) << "::control (const ClassName& contType)\n";
	    codeStream << "{\n";
	    codeStream << "\treturn (Resource*) 0;\n";
	    codeStream << "}\n\n";

	    codeStream << "void* " << typeName(CLIENT_DECL) << "::castup (const ClassName& _type) const\n";
	    codeStream << "{\n";
	    codeStream << "\tif (_type == name())\n";
	    codeStream << "\t\treturn (void*) this;\n";
	    codeStream << "\telse\n";
	    codeStream << "\t\treturn NULL;\n";
	    codeStream << "}\n\n";

	    codeStream << typeName(CLIENT_DECL) << "* " << typeName(CLIENT_DECL) << "::castup (Resource* toCast)\n";
	    codeStream << "{\n";
	    codeStream << "\tif (toCast == (Resource*) 0) // what about multiple inheritance!!\n";
	    codeStream << "\t\treturn NULL;\n";
	    codeStream << "\telse\n";
	    codeStream << "\t\treturn (" << typeName(CLIENT_DECL) << "*) toCast->castup(name());\n";
	    codeStream << "}\n";
	}
	else
	    stubOk = FALSE;
...
{
    if (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE)
	return produceArjunaIIServerCode(codeStream);
...
Boolean ClassType::produceArjunaIIServerCode ( ostream& codeStream )
{
    if (classFullDecl)
	return classFullDecl->produceArjunaIIServerCode(codeStream);
    else
    {
	Boolean stubOk = TRUE;
	Declaration *decl;
	...
	if (classStubLoc != STUB_NOREMOTE)
	{
	    ...
	    codeStream << "ClassName* " << typeName(SERVER_DECL) << "::_className = NULL;\n";
	    
	    printArjunaIIServerSpecials(codeStream);
...
Boolean ClassType::produceArjunaIIInterfaceCode ( ostream& codeStream )
{
    return FALSE;
}
