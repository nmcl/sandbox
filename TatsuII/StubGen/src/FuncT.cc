 ...
 * UK.
 *
 * $Id: FuncT.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: FuncT.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $";
...
Boolean FctType::freezeType ()
{
    ...
    if ((exceptionSpec == 0) &&
	((GlobalInfo::getOutputStyle() == CPLUSPLUS_STYLE) || (GlobalInfo::getOutputStyle() == JAVA_STYLE) || (GlobalInfo::getOutputStyle() == CPLUSPLUS_AND_JAVA_STYLE) || (GlobalInfo::getOutputStyle() == ARJUNAII_STYLE)) &&
				    ...
				    Boolean noConst)
{
	...
	{
	    s << "RPC::OperationOutcome " << funcServerName;	    
	    s << " (RpcBuffer&, RpcBuffer&)";
	}
	else
	{
...
void FctType::declareClientLocals ( ostream& s )
{
    ...
    {
	s << "Dispatcher::Outcome _rpcStatus_ = Dispatcher::UNKNOWN;\n";
	s << "RPC::OperationOutcome _serverStatus_ = RPC::OPER_INVOKED_OK;\n\n";
	
	s << "Array<Buffer*> work(1);\n";
	s << "Array<Buffer*> *result = new Array<Buffer*>(1);\n\n";

	s << "work[0] = &_callBuffer_;\n";
	s << "(*result)[0] = &_replyBuffer_;\n\n";
    }
...
void FctType::doCallToServer ( ostream& s )
{
	...
	s << "if (_dispatcher)\n\t_rpcStatus_ = _dispatcher->dispatch(work, result);\n";
}
...
Boolean FctType::printClientCode ( ostream& s )
{
	...
	while (blist)
	{
		...
		s << cl->typeName(CLIENT_DECL) << "(_objName_, RpcBuffer())";
	...
	if (GlobalInfo::getOutputStyle() != ARJUNAII_STYLE)
	    ...
	    s << "_dispatcher(0), _clientManager(_objName_), _myHashVal(-1), _theHandler(0)";
    }
    ...
    {
	/*
	 * First create the dispatcher. We may need it when
	 * creating the type of RpcBuffer via createBuffer.
	 */
    
	if (ftype == CONSTRUCTOR)
	{
	    s << "_dispatcher = new Dispatcher(_clientManager);\n";
	    s << "Resource::ref(_dispatcher);\n\n";
	}
    ...
    {
	s << "\nif (_rpcStatus_ == Dispatcher::DONE)\n";
	s << "{\n";
	s << "\tif (result.getSize() > 1)\n";
	s << "\t\tcerr << \"WARNING: \" << result.getSize()-1 << \" excess reply buffers received.\" << endl;\n";
    }
	...
	if (funcIsmemberOf->membersUseExceptions())
	{
		...
		s << "this->rpcAbort(_rpcStatus_, _serverStatus_);\nbreak;\n";
	}
    }
    ...
    {
	s << "default:\nthis->rpcAbort(_rpcStatus_, _serverStatus_);\n";
	s << "}\n";
	s << "}\nelse\nthis->rpcAbort(_rpcStatus_, _serverStatus_);\n\n";	

	/*
	 * If ArjunaII then tidy up any unexpected buffers.
	 */
    
	s << "if (result.getSize() > 1)\n{\n";
	s << "\tfor (array_t i = 1; i < result->getSize(); i++)\n";
	s << "\t\tResource::unref(result[i]);\n}\n";
	s << "delete result;\n\n";

	if (ftype == DESTRUCTOR)
	{
	    s << "Resource::unref(_dispatcher);\n";
	    s << "if (_theHandler)\n\tdelete _theHandler;\n";
	}
    }
...
Boolean FctType::printServerCode ( ostream& s )
{
    ...
    {
	s << "RPC::OperationOutcome ";                       /* return type */
	s << funcIsmemberOf->typeName(SERVER_DECL) << "::"; /* class :: */
	s << funcServerName << " (";
	s << "RpcBuffer& _work_, RpcBuffer& _result_)";
    }
	...
	s << "RPC::OperationOutcome _errCode_ = RPC::OPER_NOT_INVOKED;\n";
    ...
    else
    {
		...
		s << "theRealObject = new " << funcIsmemberOf->typeName(ARJUNAII_SERVER_DECL);		
        }
	else
        {                               /* normal operation */
            if (hasNonVoidResult)
