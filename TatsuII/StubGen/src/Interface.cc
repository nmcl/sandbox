 ...
 * UK.
 *
 * $Id: Interface.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: Interface.cc,v 1.3 1997/06/12 20:06:18 nmcl Exp $";
...
void InterfaceDef::produceAllOutput ( OutputStyle inStyle )
{
	...
	break;
    case ARJUNAII_STYLE:
	break;           // no need for _stub file in ArjunaII
    case CPLUSPLUS_STYLE:
    case SHADOWS_STYLE:
    case CPLUSPLUS_AND_JAVA_STYLE:
    ...
    /*
     * Don't have a valid stub header output stream if we are only
     * producing java!
     */
    
    if (headerFname != NullString)
...
void InterfaceDef::produceOutput ( ostream& onStream, OutputStyle inStyle )
{
    ...
    /*
     * No need for the concept of _stub.h file in ArjunaII.
     */
    
    if ((inStyle != JAVA_STYLE) &&
	(inStyle != ARJUNAII_STYLE))
    {
	...
	if (currentLoc.getFileName() == interfaceName)
	{
	    ...
	    {
		if (linkageOpen && inStyle != JAVA_STYLE)
		{
		    onStream << "\n}\n";
		}
