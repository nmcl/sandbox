 ...
 * UK.
 *
 * $Id: c++21.gram.y,v 1.1 1997/06/09 19:52:25 nmcl Exp $
 */
...
static const char RCSid[] = "$Id: c++21.gram.y,v 1.1 1997/06/09 19:52:25 nmcl Exp $";
...
/* Reserved words we currently ignore */

%token	ASM BREAK CASE CONTINUE DEFAULT DO ELSE FOR GOTO IF RETURN
	SWITCH WHILE
...
%token  BOOL_TYPE BOOL_TRUE BOOL_FALSE
...
%token	CONST VOLATILE MUTABLE

/* Exception handling */

%token  CATCH THROW TRY
/*
...
%type	<decl>
	...
	abstract.declarator.opt abstract.declarator direct.abstract.declarator
        ...
        nospec.member.declarator
	type.name
	dname id.dname
	member.function.definition constructed.id.declarator
	function.definition template.definition
    	template.class.id
...
%type	<decllist>
	...
	member.declarator.list
        nospec.member.declarator.list
	template.decl.arglist template.inst.arglist
	template.head
	template.declaration
        exception.spec.opt exception.list exception.list.opt
...
%type	<declspec>
	...
	basic.type.specifiers basic.type.specifier more.basic.type.specifiers
...
top.level.declaration:
	declaration
	| function.definition
		{ $1->addStubModifier(STUB_NOMARSHALL);GlobalInfo::putIn($1); $$ = new DeclarationList($1); }
	| linkage.specification
   	| template.declaration
        | template.definition
...
unary.expression:
	...
	| unary.operator cast.expression
		{ $$ = new BinaryExpression(*$1, 0, $2); delete $1; }
...
template.head:
	...
	template.decl.arglist '>'
		{ $$ = 0; templateNest++; templateArgs->freeze();}
	;
...
basic.type.specifiers:
	...
	| basic.type.specifier more.basic.type.specifiers
		{ $$ = $2->mergeSpecInto($1); delete $2; }
	;
...
basic.type.specifier:
	basic.type
		{ $$ = new DeclSpecifier($1); }
        | untyped.decl.specifiers basic.type
                { $1->adjustBasicType($2); $$ = $1; }
        ;

more.basic.type.specifiers:
        basic.type
                { $$ = new DeclSpecifier($1); }
        | untyped.decl.specifier
        | more.basic.type.specifiers basic.type
                { $1->adjustBasicType($2); $$ = $1; }
        | more.basic.type.specifiers untyped.decl.specifier
                { $$ = $2->mergeSpecInto($1); delete $2; }
        ;
...
param.type.list:
        '(' argument.declaration.list ')' cv.qualifier.list.opt exception.spec.opt pure.specifier.opt
            {
		$$ = new FctType(currentClassType,$2,$4,$5);
		if ($6)
		{
		    $$->makePure();
		    delete $6;
...
template.decl.arglist:
	template.decl.arglist ',' template.decl.arg
		{ templateArgs->putIn($3); $3->freezeDecl(); $$ = 0;}
	| template.decl.arg
		{ templateArgs->putIn($1); $1->freezeDecl(); $$ = 0; }
	;
...
function.definition:
	...
	|  untyped.decl.specifiers nested.scope constructor ctor.initializer.opt function.body
		{    
		    Declaration *decl = $3;
	...
	| nested.scope constructor ctor.initializer.opt function.body
		{ 
		    Declaration *decl = $2;
		    ...
		    exitTemporaryNest();
    		    decl->makeQualified($1);
		    decl->setInit($3);
		    decl->setBody($4);
		    $$ = decl;
		}
	|  untyped.decl.specifiers nested.scope destructor function.body
		{    
		    Declaration *decl = $3;
	...
	| nested.scope destructor function.body
		{ 
		    Declaration *decl = $2;
	...
	| untyped.decl.specifiers id.declarator function.body
        ...
        | nested.scope conversion.operator function.body
		{ 
		    $2->setBody($3);
                    exitTemporaryNest();
		    $2->makeQualified($1);
...
template.definition:
stub.decl.specifiers.opt template.head function.definition
                {
                    if ($1)
                    {
                        $1->finalise();
		    ...
		    $3->addTemplateArgs(templateArgs);
		    templateNest--; templateArgs = 0;
		    GlobalInfo::popContext();
		    GlobalInfo::putIn($3);
		    $$ = $3;
...
member.list:
	...
	| access.specifier ':'
        ...
        | member.list access.specifier ':'
                {
			currentClassType->changeVisibility($2);
			$$ = $1;
		}
...
member.declaration:
	...
	| untyped.decl.specifiers nospec.member.declarator.list ';'
		{ 
		    $1->finalise();
	...
	| member.function.definition semi.opt
		{ $1->addStubModifier(STUB_NOMARSHALL);$1->setVisibility(currentClassType->getVisibility()); $$ = 0; }
	| qualified.name ';'
		{
...
constructor:
	CLASSTYPE '(' arg.declaration.list.opt ')' cv.qualifier.list.opt exception.spec.opt pure.specifier.opt
		{
...
destructor:
	 '~' CLASSTYPE void.parameter.list exception.spec.opt 
		{
		    String name = $2->getName();
	 ...
	 | '~' IDENTIFIER void.parameter.list exception.spec.opt 
               {
		   String dname = *$2;
...
member.declarator:
	declarator 
	| constructor
	        { if (isCtor) currentClassType->addCtor($1); }
	| destructor
		{ currentClassType->addDtor($1); }
	| conversion.operator
...
nospec.member.declarator.list:
	...
	| nospec.member.declarator.list ',' nospec.member.declarator
		{
		    $1->appendDecl($3);
		}
	;
...
conversion.operator:
        ...
        OPERATOR conversion.type.name void.parameter.list cv.qualifier.list.opt exception.spec.opt pure.specifier.opt
		{
		    String name = $2->typeName(NORMAL_DECL);

		    FctType *cFunc = new FctType(currentClassType, 0, $4,
						 $5, CONVERSION);
		    cFunc->attachBaseType($2);
		    cFunc->addName(name);
...
mem.initializer:
	...
	| IDENTIFIER  '(' expression.list ')'
		{ $$ = new BinaryExpression("",
					    new Literal(*$1),
					    new NestedExpression($3)); delete $1; }
	| IDENTIFIER  '(' ')'
		{ $$ = new Literal(*$1 + "()"); delete $1; }
	;
