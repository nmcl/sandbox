...
/*
 * $Id: RelPort.cc,v 1.12 1996/02/12 14:42:24 nmcl Exp $
 */
...
ReliablePort::RPOutcome ReliablePort::acceptConnection ( PortAddress& from )
{
    ...
    if (msgSock == -1)
    {
	if (pollForMessage(isMessage) == DONE)
	{
...
ReliablePort::RPOutcome ReliablePort::setupConnection ( const PortAddress& to )
{
	...
	{
	    if (::connect(sock, (struct sockaddr*) &_sockAddr, length) == 0)
	    {
		isConnected = TRUE;
		return DONE;
	    }
						   ...
						   int number)
{
    ...
    RPOutcome res;
    int sockUse = ((msgSock == -1) ? sock : msgSock);
...
ReliablePort::RPOutcome ReliablePort::pollForMessage ( Boolean& isMess ) const
{
    RPOutcome outcome;
